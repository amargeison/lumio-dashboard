...
export async function generateMetadata(
  ...
  let manifestHref = `/darts/${slug}/m/anon/manifest.webmanifest`
  ...
  try {
    ...
    if (error) {
      ...
      try {
        ...
        manifestHref = `/darts/${slug}/m/${cacheBuster}/manifest.webmanifest?install_token=${encodeURIComponent(token)}`
        debugReason = 'minted'
      } catch (e) {
  ...
  console.log('[layout-meta] ' + JSON.stringify({ sport: 'darts', slug, hasUser: debugReason === 'minted', mintedToken: debugReason === 'minted', reason: debugReason }))
