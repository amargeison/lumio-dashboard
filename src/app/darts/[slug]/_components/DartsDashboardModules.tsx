...
import {
  DARTS_ORG, DARTS_FIXTURES, DARTS_TODAY, DARTS_AI_BRIEF, DARTS_INBOX,
  DARTS_RECENTS, DARTS_PERF_INTEL, DARTS_FORM,
  DARTS_TOP_STATS, DARTS_MY_TEAM,
  type DtAIBriefItem, type DtInboxChannel, type DtFixture, type DtTeamMember,
} from '../_lib/darts-dashboard-data'
...
export function HeroToday({
  ...
  const f = DARTS_FIXTURES[0]
  const [counter, setCounter] = useState({ h: 9, m: 47, s: 12 })
  ...
  return (
    <Card T={T} density={density} style={{ gridColumn: '1 / span 8', overflow: 'hidden', padding: `${density.pad}px ${density.pad + 4}px` }}>
      <svg style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', opacity: T.isDark ? 0.10 : 0.05, pointerEvents: 'none' }}>
        <defs>
          <pattern id="dt-hero-ptn" x="0" y="0" width="44" height="44" patternUnits="userSpaceOnUse">
            <path d="M0 44 L44 0" stroke={T.text} strokeWidth="0.5" />
          </pattern>
        </defs>
        <rect width="100%" height="100%" fill="url(#dt-hero-ptn)" />
      </svg>
      ...
      <div style={{ position: 'relative', display: 'flex', alignItems: 'flex-start', gap: 18 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10, flexWrap: 'wrap' }}>
            ...
            <span style={{ fontSize: 10.5, color: T.text3, letterSpacing: '0.12em', textTransform: 'uppercase', fontFamily: FONT_MONO }}>PDC · {f.comp}</span>
            <span style={{ marginLeft: 'auto', fontSize: 10, color: '#F59E0B', fontFamily: FONT_MONO, padding: '2px 8px', borderRadius: 4, background: 'rgba(245,158,11,0.14)', border: '1px solid rgba(245,158,11,0.35)', fontWeight: 700, letterSpacing: '0.08em' }}>{f.format}</span>
          </div>
          <h1 style={{ margin: 0, fontFamily: FONT, fontSize: density.h1 + 4, fontWeight: 600, color: T.text, letterSpacing: '-0.02em', lineHeight: 1.1 }}>
            {DARTS_ORG.playerShort} <span style={{ color: T.text3, fontWeight: 400 }}>vs</span> {f.opp}
          </h1>
          {quote && (
          ...
          <div style={{ display: 'flex', gap: 18, marginTop: 12, fontSize: 12, color: T.text2, flexWrap: 'wrap' }}>
            <div><span style={{ color: T.text3 }}>Walk-on</span> <span className="tnum" style={{ color: T.text, fontFamily: FONT_MONO, marginLeft: 6 }}>{f.time}</span></div>
            <div><span style={{ color: T.text3 }}>Venue</span> <span style={{ color: T.text, marginLeft: 6 }}>{f.venue}</span></div>
            <div><span style={{ color: T.text3 }}>H2H</span> <span className="tnum" style={{ color: T.good, marginLeft: 6, fontWeight: 600, fontFamily: FONT_MONO }}>2-1</span></div>
            <div><span style={{ color: T.text3 }}>Prize</span> <span style={{ color: T.text, marginLeft: 6, fontFamily: FONT_MONO }}>{f.prize}</span></div>
            <div><span style={{ color: T.text3 }}>Avg</span> <span style={{ color: T.text, marginLeft: 6, fontFamily: FONT_MONO }}>{DARTS_ORG.threeDartAvg}</span></div>
          </div>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4, color: T.text2, fontSize: 12 }}>
          <div className="tnum" style={{ color: T.text, fontSize: 13 }}>{DARTS_ORG.date}</div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}><Icon name="cloud" size={13} stroke={1.5} /> 14° · clear</div>
          <div className="tnum" style={{ display: 'flex', alignItems: 'center', gap: 6, color: T.warn }}>
            <Icon name="cloud" size={13} stroke={1.5} /> Indoor
          </div>
          ...
          <div style={{ fontSize: 10, color: T.text3, fontFamily: FONT_MONO, letterSpacing: '0.06em' }}>TO THROW</div>
        </div>
      </div>
...
export function TodaySchedule({ T, accent, density }: Common) {
  return (
    <Card T={T} density={density} hover style={{ gridColumn: '9 / span 4' }}>
      <SectionHead T={T} title="Today" right={<span className="tnum" style={{ fontFamily: FONT_MONO }}>{DARTS_ORG.date.split(',')[1]?.trim() ?? DARTS_ORG.date}</span>} />
      <div style={{ position: 'relative' }}>
        <div style={{ position: 'absolute', left: 49, top: 6, bottom: 6, width: 1, background: T.border }} />
        {DARTS_TODAY.map((it, i) => (
...
export function StatTiles({ T, accent, density }: Common) {
  const tiles = DARTS_TOP_STATS.map(s => ({
...
export function AIBrief({ T, accent, density, onAsk }: Common & { onAsk?: () => void }) {
  const [items, setItems] = useState<(DtAIBriefItem & { dismissed?: boolean })[]>(DARTS_AI_BRIEF.map(x => ({ ...x })))
...
function InboxRow({ T, c, first }: { T: ThemeTokens; c: DtInboxChannel; first: boolean }) {
  const [h, setH] = useState(false)
  return (
...
export function Inbox({ T, density }: Common) {
  ...
  const items = filter === 'urgent' ? DARTS_INBOX.filter(c => c.urgent) : DARTS_INBOX
  const tabs: [typeof filter, string, number][] = [['all', 'All', DARTS_INBOX.length], ['urgent', 'Urgent', DARTS_INBOX.filter(c => c.urgent).length]]
  return (
...
export function MyTeam({ T, accent, density }: Common) {
  const dot = (status: DtTeamMember['status']) => status === 'green' ? T.good : status === 'amber' ? T.warn : T.bad
  return (
    <Card T={T} density={density} style={{ gridColumn: '10 / span 3' }}>
      <SectionHead T={T} title="My team" right={<span style={{ fontFamily: FONT_MONO }}>{DARTS_MY_TEAM.length}</span>} />
...
function FixtureCard({ T, accent, f, hot, onClick }: { T: ThemeTokens; accent: AccentTokens; f: DtFixture; hot: boolean; onClick?: () => void }) {
  const [h, setH] = useState(false)
  return (
    <div onClick={onClick} onMouseEnter={() => setH(true)} onMouseLeave={() => setH(false)}
      style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10, color: T.text3, letterSpacing: '0.04em', textTransform: 'uppercase' }}>
        ...
        <span style={{ fontSize: 9, padding: '1px 5px', borderRadius: 3, background: `${accent.hex}22`, color: accent.hex, fontWeight: 700, letterSpacing: '0.06em' }}>{f.format}</span>
      </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginTop: 'auto' }}>
        <div className="tnum" style={{ fontSize: 11, color: hot ? accent.hex : T.text2, fontFamily: FONT_MONO }}>{f.prize}</div>
...
export function Fixtures({ T, accent, density, onPick }: Common & { onPick?: (f: DtFixture) => void }) {
  return (
    <Card T={T} density={density} style={{ gridColumn: '1 / span 7' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10 }}>
        {DARTS_FIXTURES.slice(0, 4).map((f, i) => <FixtureCard key={i} T={T} accent={accent} f={f} hot={i === 0} onClick={() => onPick?.(f)} />)}
      </div>
    </Card>
  )
}
...
// ─── Season standing — PDC rank + earnings + form ────────────────────

export function Season({ T, accent, density }: Common) {
  const s = DARTS_ORG.season
  return (
    <Card T={T} density={density} style={{ gridColumn: '8 / span 5', display: 'flex', flexDirection: 'column' }}>
      <SectionHead T={T} title="Season standing" right={<span>{DARTS_ORG.tour} Order of Merit</span>} />
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 14 }}>
        <span style={{ fontSize: 10.5, color: T.text3, letterSpacing: '0.06em', textTransform: 'uppercase' }}>PDC</span>
        <span className="tnum" style={{ fontSize: 36, fontWeight: 500, letterSpacing: '-0.03em', color: accent.hex, fontFamily: FONT }}>
          #{s.position}
        </span>
        <span style={{ fontSize: 10.5, color: T.good, fontWeight: 600 }}>↑2</span>
        <span className="tnum" style={{ fontSize: 11, color: T.text3, marginLeft: 'auto', fontFamily: FONT_MONO }}>£{Math.round(s.prizeMoney/1000)}k · {s.oneEighties} 180s</span>
      </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: T.text2 }}>
        ...
        <span style={{ color: T.text3 }}>Avg {DARTS_ORG.threeDartAvg}</span>
      </div>
    </Card>
  )
}
