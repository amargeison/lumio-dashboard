...
function buildSvorData() {
  return PUPILS.map((p: any) => {
    ...
    const concern = x < 90 && y < 90 ? 'clear' : (x < 90 || y < 90) ? 'slight' : 'none'
...
function svorQuadrant(d: any) {
  return d.x >= 90 && d.y >= 90 ? 'Typical reading and language skills' : d.x < 90 && d.y < 90 ? 'Poor reading and poor language skills' : d.y < 90 ? 'Poor language skills' : 'Poor reading skills'
}
function SvorTooltip({ active, payload }: any) {
  ...
  return (
    <div style={{ background: 'white', border: `1px solid ${T.border}`, borderRadius: 8, padding: '8px 12px', fontSize: 12 }}>
      ...
      <p style={{ margin: '0 0 2px', color: T.text }}>Language: {d.y}</p>
      <p style={{ margin: '0 0 2px', color: T.text }}>Decoding: {d.x}</p>
      <p style={{ margin: '0 0 4px', color: '#1D4ED8', fontStyle: 'italic', fontSize: 11 }}>{svorQuadrant(d)}</p>
      ...
      {p?.neli && <span style={{ display: 'inline-block', marginTop: 4, padding: '2px 8px', borderRadius: 4, fontSize: 10, fontWeight: 700, background: T.goldLight, color: T.gold }}>TEL TED</span>}
    </div>
  )
}
...
function SvorReport() {
  ...
  return (
    <div style={{ maxWidth: 800, margin: '0 auto' }}>
      ...
      {/* Print footer */}
      <div className="neli-print-footer" style={{ borderTop: `2px solid ${T.border}`, paddingTop: 16, marginTop: 16 }}>
      ...
      <div className="neli-no-print" style={{ textAlign: 'right', marginTop: 16 }}>
...
function ReportsPanel() {
  ...
  function handlePrint() {
    ...
    s.textContent = '@media print{.neli-sidebar,.neli-report-left,.neli-header,.neli-no-print{display:none!important}.neli-report-preview{position:absolute!important;inset:0!important}select,button{display:none!important}.neli-print-footer{display:block!important}}'
