...
export function getNeedsPlan(added: TodaySession[]): Booking[] {
  return BOOKINGS
...
export function bookingCalItems(): CalItem[] {
  return BOOKINGS.map(b => ({
    key: b.id, date: b.date, start: b.start, end: b.end,
    player: b.player, court: b.court, type: b.type, status: b.status, bookingId: b.id,
  }))
}
...
// any manually-added sessions that aren't tied to a booking.
export function getCalendarItems(added: TodaySession[]): CalItem[] {
  const fromBookings: CalItem[] = BOOKINGS.map(b => ({
    key: b.id, date: b.date, start: b.start, end: b.end,
    player: b.player, court: b.court, type: b.type, status: b.status,
    bookingId: b.id, sessionId: sessionForBooking(b.id, added)?.id,
  }))
  const manual: CalItem[] = added.filter(s => !s.bookingId).map(s => ({
    key: s.id, date: s.date, start: s.time, end: s.end,
    player: s.player, court: s.court, type: s.type, status: 'confirmed' as const, sessionId: s.id,
  }))
