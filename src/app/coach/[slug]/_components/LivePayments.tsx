    ...
    <div style={{ fontFamily: FONT }}>
      ...
      <div style={{ background: T.panel, border: `1px solid ${T.border}`, borderRadius: 12, padding: 16 }}>
        <div style={{ display: 'flex', alignItems: 'baseline', marginBottom: 12, flexWrap: 'wrap', gap: 8 }}>
          ...
          <span style={{ fontSize: 11.5, color: T.text3 }}>Tap a player to update their sessions.</span>
        ...
        {payments.rows.length === 0 ? <div style={{ fontSize: 12.5, color: T.text3 }}>No packages assigned yet.</div> : (
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 640 }}>
              ...
              <tbody>
                {payments.rows.map(p => {
                  const s = statusOf(p); const used = p.sessions_used || 0; const total = p.sessions_total || 0
                  const pct = total ? Math.min(100, used / total * 100) : 0
                  const bar = s === 'overdue' ? T.bad : accent.hex
                  return (
                    <tr key={p.id} onClick={() => setEditPay(p)} style={{ borderTop: `1px solid ${T.border}`, cursor: 'pointer' }}>
                      <td style={{ padding: '10px 10px', fontSize: 12.5, color: T.text, fontWeight: 600 }}>{p.player_name || '—'}</td>
                      <td style={{ padding: '10px 10px', fontSize: 12, color: T.text2 }}>{p.item || '—'}</td>
                      ...
                      <td style={{ padding: '10px 10px' }}><span style={{ fontSize: 9.5, fontWeight: 700, color: statusColour(s), background: `${statusColour(s)}22`, padding: '2px 7px', borderRadius: 4, textTransform: 'uppercase' }}>{s}</span></td>
                      <td style={{ padding: '10px 10px', fontSize: 12, color: T.text2 }}>{fmtD(p.renews_date)}</td>
                    </tr>
                  )
                })}
              </tbody>
      ...
      {editPay && <AssignForm T={T} accent={accent} players={players} packages={packages.rows} pay={editPay === 'new' ? null : editPay}
        onClose={() => setEditPay(null)}
        onDelete={editPay !== 'new' ? async () => { await payments.remove(editPay.id); setEditPay(null) } : undefined}
        onSave={async v => { if (editPay === 'new') await payments.add(v); else await payments.edit(editPay.id, v); setEditPay(null) }} />}
    </div>
  )
}
...
function AssignForm({ T, accent, players, packages, pay, onClose, onSave, onDelete }: { T: ThemeTokens; accent: AccentTokens; players: Player[]; packages: Pkg[]; pay: Pay | null; onClose: () => void; onSave: (v: Record<string, any>) => Promise<void>; onDelete?: () => Promise<void> }) {
  const [d, setD] = useState<Record<string, any>>({ player_name: pay?.player_name || '', item: pay?.item || '', amount: pay?.amount ?? '', sessions_used: pay?.sessions_used ?? 0, sessions_total: pay?.sessions_total ?? '', status: pay?.status || 'active', renews_date: pay?.renews_date || '' })
  const [saving, setSaving] = useState(false)
  const set = (k: string, v: any) => setD(p => ({ ...p, [k]: v }))
  ...
  const save = async () => { if (!String(d.player_name).trim() || saving) return; setSaving(true); try { await onSave({ player_name: d.player_name, item: d.item, amount: Number(d.amount) || null, sessions_used: Number(d.sessions_used) || 0, sessions_total: Number(d.sessions_total) || null, status: d.status, renews_date: d.renews_date || null }) } finally { setSaving(false) } }
  return (
    <Shell T={T} title={pay ? 'Update package' : 'Assign package'} onClose={onClose}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
        ...
        <div><label style={lab(T)}>Used</label><input type="number" value={d.sessions_used} onChange={e => set('sessions_used', e.target.value)} style={field(T)} /></div>
        <div><label style={lab(T)}>Total</label><input type="number" value={d.sessions_total} onChange={e => set('sessions_total', e.target.value)} style={field(T)} /></div>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
