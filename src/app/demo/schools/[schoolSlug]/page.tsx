...
import {
  ...
  Settings, Bell, Menu, X, Activity, Calendar, Phone,
  AlertTriangle, CheckCircle2, Clock, Loader2, TrendingUp, Zap,
} from 'lucide-react'
...
export default function DemoSchoolPage({ params }: { params: Promise<{ schoolSlug: string }> }) {
  ...
  return (
    <div className="flex h-screen overflow-hidden" style={{ backgroundColor: '#07080F' }}>
      ...
      {/* ── Sidebar ─────────────────────────────────────────────── */}
      <aside
        className={['fixed inset-y-0 left-0 z-50 flex flex-col transition-transform duration-300 md:translate-x-0', sidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'].join(' ')}
      ...
      >
        <div className="flex shrink-0 items-center justify-between px-4 py-4" style={{ borderBottom: '1px solid #1F2937' }}>
          ...
          <button className="md:hidden" onClick={() => setSidebarOpen(false)} style={{ color: '#9CA3AF' }}><X size={16} /></button>
        </div>

        <div className="px-4 py-3" style={{ borderBottom: '1px solid #1F2937' }}>
          <p className="text-xs font-semibold truncate" style={{ color: '#F9FAFB' }}>{SCHOOL.name}</p>
          <p className="text-xs" style={{ color: '#9CA3AF' }}>Primary · {SCHOOL.pupils} pupils</p>
        </div>
          ...
          {NAV.map((item, i) => {
            ...
            return (
              <div key={item.id}>
                ...
                <button
                  onClick={() => fireToast(`navigate to ${item.label}`)}
                ...
                >
                  ...
                  {item.badge !== null && (
                    <span className="flex h-4 min-w-4 items-center justify-center rounded-full px-1 text-xs font-bold"
                      style={{ backgroundColor: '#EF4444', color: '#F9FAFB', fontSize: 10 }}>
                      {item.badge}
                    </span>
                  )}
                </button>
        ...
        <div className="shrink-0 px-4 py-3 text-xs" style={{ borderTop: '1px solid #1F2937', color: '#4B5563' }}>Demo workspace</div>
      </aside>
      ...
      <div className="flex flex-1 flex-col md:pl-[200px] overflow-hidden">
        ...
        {/* Demo banner */}
        <div className="flex items-center justify-between gap-3 px-4 py-2.5" style={{ backgroundColor: 'rgba(13,148,136,0.12)', borderBottom: '1px solid rgba(13,148,136,0.2)' }}>
          <div className="flex items-center gap-2 min-w-0">
            <Zap size={13} style={{ color: '#0D9488', flexShrink: 0 }} />
            <p className="text-xs truncate" style={{ color: '#0D9488' }}>
              <span className="font-semibold">Trial workspace</span> · 14 days remaining · Demo data only · Auto-deleted after 14 days
            </p>
          </div>
          <Link href="/schools" className="shrink-0 rounded-lg px-3 py-1 text-xs font-semibold whitespace-nowrap" style={{ backgroundColor: '#0D9488', color: '#F9FAFB' }}>
            Upgrade now →
          </Link>
        </div>
        ...
        <header className="flex h-14 shrink-0 items-center gap-3 px-4 md:px-6" style={{ borderBottom: '1px solid #1F2937', backgroundColor: '#07080F' }}>
          ...
          <div className="flex flex-1 items-center gap-2 min-w-0">
            <span className="text-sm font-semibold truncate" style={{ color: '#F9FAFB' }}>{SCHOOL.name}</span>
            <span className="shrink-0 text-xs px-2 py-0.5 rounded-full" style={{ backgroundColor: 'rgba(13,148,136,0.1)', color: '#0D9488' }}>Ofsted: {SCHOOL.ofsted}</span>
            <span className="shrink-0 hidden sm:block text-xs" style={{ color: '#6B7280' }}>{SCHOOL.pupils} pupils</span>
          </div>
          <div className="flex items-center gap-3">
            <button onClick={() => fireToast('open notification panel')} className="relative" style={{ color: '#9CA3AF' }}>
              <Bell size={18} />
            ...
            <button onClick={() => fireToast('invite team members')} className="hidden sm:flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium" style={{ backgroundColor: '#111318', border: '1px solid #1F2937', color: '#9CA3AF' }}>
