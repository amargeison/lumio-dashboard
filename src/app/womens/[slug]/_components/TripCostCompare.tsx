...
function TripCard({ trip, comparison, active, onClick }: { trip: Trip; comparison: TripCostComparison; active: boolean; onClick: () => void }) {
  return (
    <button onClick={onClick} className={`w-full text-left rounded-xl border p-3 transition ${active ? 'border-pink-600/50 bg-pink-600/10' : 'border-gray-800 bg-[#0D1117] hover:border-gray-700'}`}>
  ...
  return (
    <div className="rounded-xl border border-gray-800 bg-[#0D1117] p-4">
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        {types.map((t) => (
          <button key={t} onClick={() => openDraft(t)} disabled={busy} className="rounded-lg border border-gray-700 px-2.5 py-1.5 text-xs text-gray-200 hover:border-pink-600/50 hover:text-white disabled:opacity-50">{DRAFT_LABELS[t]}</button>
        ))}
      </div>
      {sent && (
      ...
      {draft && (
        <div className="mt-4 rounded-lg border border-gray-800 bg-black/30 p-3">
          ...
          <input value={subject} onChange={(e) => setSubject(e.target.value)} className="mt-1 w-full rounded-md border border-gray-700 bg-[#0D1117] px-2.5 py-1.5 text-sm text-white outline-none focus:border-pink-600/60" />
          <label className="mt-3 block text-[11px] text-gray-500">Message (editable)</label>
          <textarea value={body} onChange={(e) => setBody(e.target.value)} rows={12} className="mt-1 w-full rounded-md border border-gray-700 bg-[#0D1117] px-2.5 py-2 text-xs leading-relaxed text-gray-200 outline-none focus:border-pink-600/60 font-mono" />
      ...
      <div className="w-full max-w-lg rounded-xl border border-gray-800 bg-[#0D1117] p-5 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mt-4 grid grid-cols-2 gap-3">
          ...
          <div className="col-span-2 flex items-center gap-2">
            <input id="ov2" type="checkbox" checked={overnight} onChange={(e) => setOvernight(e.target.checked)} className="accent-pink-600" />
