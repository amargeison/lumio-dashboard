    ...
    <div style={{ display: 'grid', gap: 16 }}>
      ...
      <Card>
        ...
        <div style={{ display: 'grid', gap: 8 }}>
          {NEEDS_ATTENTION.map(item => (
            <div key={item.id} style={{ display: 'flex', gap: 10, alignItems: 'flex-start', background: item.kind === 'warn' ? '#FDF3F3' : '#F7F5F2', borderRadius: 10, padding: '10px 12px' }}>
              {item.kind === 'warn'
                ? <AlertCircle size={16} style={{ color: TP_RED, flexShrink: 0, marginTop: 1 }} />
                : <Info size={16} style={{ color: '#6B6560', flexShrink: 0, marginTop: 1 }} />}
              <div style={{ fontSize: 13, color: TP_DARK }}>{item.text}</div>
            </div>
          ))}
        </div>
      </Card>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: 16 }}>
        ...
        <Card>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12.5 }}>
            ...
            <tbody>
              {SCHOOLS.map(s => (
                <tr key={s.id} style={{ borderTop: '1px solid #F0EBE5' }}>
                  <td style={{ padding: '8px 4px', fontWeight: 700, color: TP_DARK }}>{s.name}<div style={{ fontWeight: 400, fontSize: 11, color: '#8A847E' }}>{s.borough} · {s.headTeacher}</div></td>
        ...
        <div style={{ display: 'grid', gap: 16, alignContent: 'start' }}>
          ...
          <Card>
            ...
            <div style={{ fontSize: 12.5, fontWeight: 700, color: TP_DARK, marginBottom: 8 }}>{CAMPAIGN.school}</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        ...
        <Card>
          ...
          <div style={{ display: 'grid', gap: 8 }}>
            {VENUES.map(v => (
              <div key={v.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#F7F5F2', borderRadius: 10, padding: '10px 12px' }}>
                <div>
                  <div style={{ fontSize: 13, fontWeight: 700, color: TP_DARK }}>{v.name} {v.external && <Pill tone="grey">LINKED VENUE</Pill>}</div>
