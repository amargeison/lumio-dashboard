  ...
  return (
    <Card T={T} density={density} style={{ gridColumn: '1 / span 8', overflow: 'hidden', padding: `${density.pad}px ${density.pad + 4}px` }}>
      <svg style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', opacity: T.isDark ? 0.10 : 0.05, pointerEvents: 'none' }}>
        <defs>
          <pattern id="fb-hero-ptn" x="0" y="0" width="44" height="44" patternUnits="userSpaceOnUse">
...
export function StatTiles({ T, accent, density }: Common) {
  const tiles = FOOTBALL_TOP_STATS.map(s => ({
...
export function AIBrief({ T, accent, density, onAsk }: Common & { onAsk?: () => void }) {
  const [items, setItems] = useState<(FbAIBriefItem & { dismissed?: boolean })[]>(FOOTBALL_AI_BRIEF.map(x => ({ ...x })))
...
function InboxRow({ T, c, first }: { T: ThemeTokens; c: FbInboxChannel; first: boolean }) {
  const [h, setH] = useState(false)
  return (
...
export function Inbox({ T, density }: Common) {
  ...
  const items = filter === 'urgent' ? FOOTBALL_INBOX.filter(c => c.urgent) : FOOTBALL_INBOX
  const tabs: [typeof filter, string, number][] = [['all', 'All', FOOTBALL_INBOX.length], ['urgent', 'Urgent', FOOTBALL_INBOX.filter(c => c.urgent).length]]
  return (
...
export function Squad({ T, accent, density }: Common) {
  const out = FOOTBALL_SQUAD.filter(p => p.status === 'out').length
  const doubt = FOOTBALL_SQUAD.filter(p => p.status === 'doubt').length
  const fit = FOOTBALL_SQUAD.length - out - doubt
  const gk  = FOOTBALL_SQUAD.filter(s => s.group === 'gk')
  const def = FOOTBALL_SQUAD.filter(s => s.group === 'def')
...
function FixtureCard({ T, accent, f, hot, onClick }: { T: ThemeTokens; accent: AccentTokens; f: FbFixture; hot: boolean; onClick?: () => void }) {
  const [h, setH] = useState(false)
  const compTone = f.competitionTone === 'cup' ? '#F1C40F' : accent.hex
  return (
    <div
      onClick={onClick}
      onMouseEnter={() => setH(true)}
...
export function Fixtures({ T, accent, density, onPick }: Common & { onPick?: (f: FbFixture) => void }) {
  return (
    <Card T={T} density={density} style={{ gridColumn: '1 / span 7' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10 }}>
        {FOOTBALL_FIXTURES.slice(0, 4).map((f, i) => <FixtureCard key={i} T={T} accent={accent} f={f} hot={i === 0} onClick={() => onPick?.(f)} />)}
      </div>
    </Card>
  )
}
