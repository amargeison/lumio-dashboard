...
export async function POST(req: NextRequest) {
  try {
    ...
    const body = sanitizeAiBody(await req.json(), { allowWebSearch: true })
    ...
    if (data?.usage?.input_tokens != null && data?.usage?.output_tokens != null) {
      recordSpend(data.usage.input_tokens, data.usage.output_tokens, data.model || body.model, 'boxing')
    }
    trackSportsEvent(null, 'boxing', 'ai_call', body.messages?.[0]?.content?.slice(0, 80) || 'ai_call', {
