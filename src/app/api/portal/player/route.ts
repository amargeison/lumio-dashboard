...
export async function GET() {
  ...
  const [skills, lessons, bookings, media, messages] = await Promise.all([
  ...
  return NextResponse.json({
    player: { id: player.id, name: player.name, racket_stage: player.racket_stage, level: player.level, goal: player.goal, category: player.category, age: player.age },
    skills, lessons, bookings, media, messages,
  })
}
