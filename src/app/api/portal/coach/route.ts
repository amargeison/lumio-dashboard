...
export async function GET() {
  ...
  const [bookings, sessions, skills] = await Promise.all([
    safe(db.from('coach_bookings').select('id, player_name, booking_date, start_time, court, type, status').eq('coach_id', m.academyId).ilike('assigned_coach', coach).gte('booking_date', today).order('booking_date')),
