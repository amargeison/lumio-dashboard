...
export default function ReportsPage() {
  ...
  return (
    <div className="space-y-8">
      ...
      {stats.winRate > 0 || stats.forecast > 0 ? (
        <>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {['Rep Performance', 'Lead Source ROI', 'Sales Forecast', 'Lost Deal Analysis'].map(title => (
            <div key={title} className="rounded-xl p-5" style={{ background: CRM_BG, border: `1px solid ${CRM_BORDER}` }}>
              <p className="text-sm font-semibold mb-3" style={{ color: '#F1F3FA' }}>{title}</p>
              <div className="flex items-center justify-center" style={{ height: 160 }}>
                <p className="text-sm" style={{ color: '#6B7299' }}>No data yet</p>
              </div>
            </div>
          ))}
        </div>
