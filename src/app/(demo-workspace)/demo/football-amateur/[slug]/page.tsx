...
function Sidebar({ activeDept, onSelect, open, onClose, clubName }: { activeDept: DeptId; onSelect: (d: DeptId) => void; open: boolean; onClose: () => void; clubName?: string }) {
  ...
  const sections: { label: SidebarSection; items: typeof SIDEBAR_ITEMS }[] = [
    ...
    { label: 'Club', items: SIDEBAR_ITEMS.filter(i => i.section === 'Club') },
    { label: 'Operations', items: SIDEBAR_ITEMS.filter(i => i.section === 'Operations') },
    { label: 'Resources', items: SIDEBAR_ITEMS.filter(i => i.section === 'Resources') },
    { label: 'Club Info', items: SIDEBAR_ITEMS.filter(i => i.section === 'Club Info') },
  ]
  return (
    <>
      <aside className="hidden md:flex flex-col shrink-0 z-30 transition-all duration-200" style={{ width: expanded ? 200 : 52, backgroundColor: BG, borderRight: `1px solid ${BORDER}` }} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
        <div className="flex items-center gap-2.5 px-2.5 py-3 shrink-0" style={{ borderBottom: `1px solid ${BORDER}`, minHeight: 52 }}>
          <div className="flex h-8 w-8 items-center justify-center rounded-lg text-xs font-bold shrink-0" style={{ backgroundColor: PRIMARY, color: '#fff' }}>SR</div>
          {expanded && (<><div className="flex-1 min-w-0">{clubName && <p className="text-sm font-semibold truncate" style={{ color: TEXT }}>{clubName}</p>}<p className="text-[10px] truncate" style={{ color: TEXT_SEC }}>Grassroots Portal</p></div>
            <button onClick={togglePin} className="shrink-0 p-1 rounded" style={{ color: pinned ? PRIMARY : '#4B5563', transform: pinned ? 'rotate(0deg)' : 'rotate(45deg)' }} title={pinned ? 'Unpin' : 'Pin open'}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 17v5"/><path d="M9 10.76a2 2 0 0 1-1.11 1.79l-1.78.9A2 2 0 0 0 5 15.24V17h14v-1.76a2 2 0 0 0-1.11-1.79l-1.78-.9A2 2 0 0 1 15 10.76V5a1 1 0 0 0-1-1h-4a1 1 0 0 0-1 1z"/></svg></button></>)}
        </div>
      ...
      {open && (<div className="md:hidden fixed inset-0 z-40 flex"><div className="absolute inset-0" style={{ backgroundColor: 'rgba(0,0,0,0.6)' }} onClick={onClose} />
        <aside className="relative z-50 w-56 flex flex-col" style={{ backgroundColor: BG, borderRight: `1px solid ${BORDER}` }}>
          ...
          <nav className="flex flex-1 flex-col gap-0.5 p-3 overflow-y-auto amateur-sidebar-scroll">{SIDEBAR_ITEMS.map(item => { const active = activeDept === item.id; return (
            <button key={item.id} onClick={() => { onSelect(item.id); onClose() }} className="flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm font-medium text-left w-full" style={{ backgroundColor: active ? `${PRIMARY}1f` : 'transparent', color: active ? PRIMARY : TEXT_SEC }}><item.icon size={15} strokeWidth={active ? 2.5 : 2} /><span className="truncate">{item.label}</span></button>
          )})}</nav></aside></div>)}
    </>
  )
}
...
export default function FootballAmateurDashboard({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const clubName = 'Sunday Rovers FC'
  ...
  return (
    <div className="flex flex-col" style={{ backgroundColor: '#07080F', color: TEXT, height: '100vh', overflow: 'hidden' }}>
      ...
      {/* Scrollbar styles for amateur portal */}
      <style>{`
