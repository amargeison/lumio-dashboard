...
import NonLeagueContent, { NL_SIDEBAR_ITEMS, type NLDeptId } from './nl-content'
...
type Tier = 'grassroots' | 'non-league' | 'pro'
type AnyDeptId = DeptId | NLDeptId | 'pro-view'

const TIER_CONFIG = {
  'grassroots': { primary: '#16A34A', dark: '#15803D', accent: '#BBF7D0', clubName: 'Sunday Rovers FC', league: 'Westshire Sunday League', initials: 'SR', portalLabel: 'Grassroots Portal' },
  'non-league': { primary: '#D97706', dark: '#B45309', accent: '#FDE68A', clubName: 'Harfield FC', league: 'Northern Premier League West', initials: 'HF', portalLabel: 'Non-League Portal' },
  'pro':        { primary: '#C0392B', dark: '#922B21', accent: '#F5B7B1', clubName: 'Oakridge FC', league: 'EFL Championship', initials: 'OF', portalLabel: 'Pro Club Portal' },
} as const

const PRO_SIDEBAR_ITEMS: { id: string; label: string; icon: React.ElementType; section: string | null }[] = [
  { id: 'pro-view', label: 'Full Dashboard', icon: Home, section: null },
]
...
function TierSwitcher({ tier, onTierChange, transitioning }: { tier: Tier; onTierChange: (t: Tier) => void; transitioning: boolean }) {
  const cfg = TIER_CONFIG[tier]
  const tiers: { id: Tier; label: string; color: string; tooltip: string }[] = [
    { id: 'grassroots', label: 'Grassroots', color: '#16A34A', tooltip: 'Sunday leagues, park football, junior clubs' },
    { id: 'non-league', label: 'Non-League', color: '#D97706', tooltip: 'Step 1–6, county leagues, semi-professional' },
    { id: 'pro', label: 'Pro Club', color: '#C0392B', tooltip: 'EFL and above, full-time professional clubs' },
  ]

  return (
    <div className="mb-4">
      <div className="flex items-center gap-1 p-1 rounded-xl" style={{ backgroundColor: CARD_BG, border: `1px solid ${BORDER}` }}>
        {tiers.map(t => {
          const active = tier === t.id
          return (
            <button key={t.id} onClick={() => onTierChange(t.id)} title={t.tooltip}
              className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg text-xs font-semibold transition-all"
              style={{ backgroundColor: active ? `${t.color}22` : 'transparent', color: active ? t.color : TEXT_SEC, border: active ? `1.5px solid ${t.color}55` : '1.5px solid transparent' }}>
      ...
      <div className="mt-2 px-1">
        <p className="text-[11px]" style={{ color: TEXT_SEC }}>
          Viewing as: <span className="font-semibold" style={{ color: cfg.primary }}>{cfg.clubName}</span> — {cfg.league}
        </p>
      </div>
      {transitioning && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center" style={{ backgroundColor: 'rgba(7,8,15,0.85)' }}>
          <div className="flex items-center gap-3 px-6 py-4 rounded-xl" style={{ backgroundColor: CARD_BG, border: `1px solid ${BORDER}` }}>
            <Loader2 size={18} className="animate-spin" style={{ color: cfg.primary }} />
            <span className="text-sm font-medium" style={{ color: TEXT }}>Loading {tier === 'grassroots' ? 'Grassroots' : tier === 'non-league' ? 'Non-League' : 'Pro Club'} view...</span>
          </div>
        </div>
      )}
    </div>
...
function Sidebar({ activeDept, onSelect, open, onClose, tier }: { activeDept: string; onSelect: (d: string) => void; open: boolean; onClose: () => void; tier: Tier }) {
  const cfg = TIER_CONFIG[tier]
  const tierColor = cfg.primary
  const items: { id: string; label: string; icon: React.ElementType; section: string | null }[] =
    tier === 'grassroots' ? SIDEBAR_ITEMS : tier === 'non-league' ? NL_SIDEBAR_ITEMS : PRO_SIDEBAR_ITEMS
    ...
    <>
      <aside className="hidden md:flex flex-col shrink-0 z-30 transition-all duration-200" style={{ width: expanded ? 200 : 52, backgroundColor: BG, borderRight: `1px solid ${BORDER}` }} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
        <div className="flex items-center gap-2.5 px-2.5 py-3 shrink-0" style={{ borderBottom: `1px solid ${BORDER}`, minHeight: 52 }}>
          <div className="flex h-8 w-8 items-center justify-center rounded-lg text-xs font-bold shrink-0" style={{ backgroundColor: tierColor, color: '#fff' }}>{cfg.initials}</div>
          {expanded && (<><div className="flex-1 min-w-0"><p className="text-sm font-semibold truncate" style={{ color: TEXT }}>{cfg.clubName}</p><p className="text-[10px] truncate" style={{ color: TEXT_SEC }}>{cfg.portalLabel}</p></div>
...
export default function FootballAmateurDashboard({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = use(params)
  const [tier, setTier] = useState<Tier>(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('lumio_football_tier') as Tier | null
      if (saved && ['grassroots', 'non-league', 'pro'].includes(saved)) return saved
    }
    return 'grassroots'
  })
  const [activeDept, setActiveDept] = useState<string>('overview')
  ...
  function handleTierChange(newTier: Tier) {
    ...
    if (typeof window !== 'undefined') localStorage.setItem('lumio_football_tier', newTier)
    setTimeout(() => {
      setTier(newTier)
      // Reset to default dept for the new tier
      if (newTier === 'grassroots') setActiveDept('overview')
      else if (newTier === 'non-league') setActiveDept('nl-overview')
      else setActiveDept('pro-view')
      setTransitioning(false)
    }, 400)
  }

  const deptLabel = tier === 'grassroots'
    ? SIDEBAR_ITEMS.find(d => d.id === activeDept)?.label || 'Overview'
    : tier === 'non-league'
    ? NL_SIDEBAR_ITEMS.find(d => d.id === activeDept)?.label || 'Overview'
    : 'Oakridge FC — Pro Dashboard'

  const avatarInitials = tier === 'grassroots' ? 'DH' : tier === 'non-league' ? 'MH' : 'JW'
  ...
  return (
    <div className="flex flex-col" style={{ backgroundColor: '#07080F', color: TEXT, height: '100vh', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'fixed', top: 12, right: 20, zIndex: 60, display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <button style={{ width: 36, height: 36, borderRadius: '50%', backgroundColor: cfg.primary, border: 'none', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', fontSize: 12, fontWeight: 600 }}>
          {avatarInitials}
        </button>
      </div>
      ...
      <div className="md:hidden flex items-center px-4 py-2 shrink-0" style={{ borderBottom: `1px solid ${BORDER}` }}>
        ...
        <span className="text-sm font-semibold ml-2 truncate" style={{ color: TEXT }}>{cfg.clubName}</span>
      </div>

      {/* Body: sidebar + content */}
      <div className="flex flex-1 overflow-hidden">
        <Sidebar activeDept={activeDept} onSelect={(d) => setActiveDept(d)} open={sidebarOpen} onClose={() => setSidebarOpen(false)} tier={tier} />

        <div className="flex-1 flex flex-col overflow-y-auto min-w-0">
          <main className="flex-1 p-4 sm:p-5">
            {/* Tier Switcher */}
            <TierSwitcher tier={tier} onTierChange={handleTierChange} transitioning={transitioning} />
              ...
              <>
                <div className="flex items-center justify-between mb-4">
                  <div>
                    ...
                    <p className="text-xs mt-0.5" style={{ color: TEXT_SEC }}>Sunday Rovers FC · Westshire Sunday League Div 2</p>
                  </div>
                </div>
            ...
            {tier === 'pro' && (
              <div>
                <div className="flex items-center justify-between mb-4">
                  <div>
                    <h1 className="text-lg font-bold">Oakridge FC — EFL Championship</h1>
                    <p className="text-xs mt-0.5" style={{ color: TEXT_SEC }}>Full professional club dashboard</p>
                  </div>
                </div>
                <div className="rounded-xl overflow-hidden" style={{ border: `1px solid ${BORDER}`, height: 'calc(100vh - 200px)' }}>
                  <iframe
                    src="/demo/football/oakridge-fc"
                    className="w-full h-full"
                    style={{ border: 'none', backgroundColor: '#07080F' }}
                    title="Oakridge FC Pro Dashboard"
                  />
                </div>
              </div>
            )}
