...
import {
  ...
  Star, MapPin, Mail, Phone, Globe, ChevronDown, ChevronUp,
} from 'lucide-react'
...
function Stars({ rating }: { rating: number }) {
  return (
    <div className="flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map(i => (
        <Star key={i} className={`w-3.5 h-3.5 ${i <= Math.round(rating) ? 'text-yellow-400 fill-yellow-400' : 'text-[#374151]'}`} />
      ))}
    </div>
  )
}
...
export default function DemoEventsPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  async function startResearch() {
    ...
    const logs = [
      ...
      `Found ${MOCK_VENUES.length} venues — loading results.`,
    ]
  ...
  return (
    <div className="min-h-screen text-[#F9FAFB] px-6 py-8 max-w-4xl mx-auto" style={{ backgroundColor: '#07080F' }}>
      ...
      <div className="mb-4 inline-flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-medium"
        ...
        Demo workspace — no real enquiry will be sent
      </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                  <button onClick={() => setHeadcount(h => Math.max(2, h - 5))}
                  ...
                  <div className="flex-1 bg-[#07080F] border border-[#1F2937] rounded-lg px-3 py-2 text-sm text-center text-[#F9FAFB]">{headcount}</div>
                  <button onClick={() => setHeadcount(h => Math.min(500, h + 5))}
                    className="w-9 h-9 rounded-lg border border-[#1F2937] flex items-center justify-center text-[#9CA3AF] hover:text-[#F9FAFB] hover:border-[#374151] transition-colors text-lg">+</button>
                </div>
              </div>
              <div>
          ...
          <div className="bg-[#111318] border border-[#1F2937] rounded-xl p-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              {REQUIREMENTS.map(req => (
                <button key={req.id} onClick={() => toggleRequirement(req.id)}
      ...
      {step === 2 && (
        <div className="space-y-4">
          <div className="flex items-center justify-between mb-2">
            <h2 className="font-semibold">{MOCK_VENUES.length} venues found</h2>
          ...
          {MOCK_VENUES.map((venue, i) => {
            const isExpanded = expanded === i; const isSelected = selected === i
            return (
              <div key={i} className={`bg-[#111318] border rounded-xl overflow-hidden transition-all ${isSelected ? 'border-teal-500' : 'border-[#1F2937]'}`}>
                <div className="p-5">
                  ...
                  {isExpanded && (
                    <div className="mt-4 pt-4 border-t border-[#1F2937] flex flex-wrap gap-4 text-sm text-[#9CA3AF]">
                      ...
                      <div className="flex items-center gap-1.5"><MapPin className="w-3.5 h-3.5" /> {venue.location}</div>
                    </div>
                  )}

                  <div className="mt-4 flex justify-end">
                    <button onClick={() => setSelected(isSelected ? null : i)}
          ...
          <div className="flex justify-between pt-2">
            ...
            <button onClick={() => setStep(3)} disabled={selected === null}
          ...
          {!booked ? (
            <>
              <div className="bg-[#111318] border border-teal-500/30 rounded-xl p-5">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-lg bg-[#1F2937] flex items-center justify-center text-xl">{selectedVenue.image}</div>
                  <div>
              ...
              <div className="bg-[#111318] border border-[#1F2937] rounded-xl p-6">
                ...
                <textarea defaultValue={emailDraft} rows={18}
              ...
              <div className="flex justify-between">
                ...
                <button onClick={() => { setBooked(true); setToast('Demo mode — in your live workspace this would send a real enquiry') }}
            ...
            <div className="bg-[#111318] border border-teal-500/30 rounded-xl p-10 text-center space-y-4">
              ...
              <p className="text-[#9CA3AF] text-sm max-w-sm mx-auto">
                In your live workspace, an enquiry would be sent to{' '}
                <span className="text-[#F9FAFB]">{selectedVenue.name}</span>.
                Most venues respond within 24 hours.
              </p>
              <div className="flex gap-3 justify-center pt-2">
                <button onClick={() => {
                  setStep(0); setEventType(''); setLocation(''); setCustomLocation(''); setHeadcount(20)
                  setBudget(''); setEventDate(''); setRequirements([]); setResearchLog([])
                  setResearchDone(false); setSelected(null); setBooked(false)
