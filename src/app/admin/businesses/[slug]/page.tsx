...
export default function BusinessDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  async function deleteAccount() {
    ...
    await fetch(`/api/admin/accounts/${slug}?type=business`, { method: 'DELETE', headers: { 'x-admin-token': token } })
      ...
      <div className="rounded-xl overflow-hidden" style={{ backgroundColor: '#111318', border: '1px solid rgba(239,68,68,0.3)' }}>
        ...
        <div className="p-5 space-y-4">
          ...
          <div style={{ borderTop: '1px solid #1F2937', paddingTop: 16 }}>
            ...
            <p className="text-xs mb-3" style={{ color: '#6B7280' }}>Type &quot;{account.company_name}&quot; to confirm</p>
            <div className="flex gap-2">
              <input value={confirmDelete} onChange={e => setConfirmDelete(e.target.value)} placeholder={account.company_name}
                className="flex-1 rounded-lg px-3 py-2 text-sm" style={{ backgroundColor: '#0A0B10', border: '1px solid #374151', color: '#F9FAFB' }} />
              <button onClick={deleteAccount} disabled={confirmDelete !== account.company_name}
