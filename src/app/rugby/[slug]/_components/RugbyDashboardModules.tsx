...
import { RUGBY_QUOTES, getDailyQuote } from '@/lib/sports-quotes'
import {
      ...
      <div style={{ position: 'relative', display: 'flex', alignItems: 'flex-start', gap: 18 }}>
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
            ...
            <span style={{ fontSize: 10.5, color: T.text3, letterSpacing: '0.12em', textTransform: 'uppercase', fontFamily: FONT_MONO }}>{f.comp} · Round 24</span>
          </div>
          <h1 style={{ margin: 0, fontFamily: FONT, fontSize: density.h1 + 4, fontWeight: 600, color: T.text, letterSpacing: '-0.02em', lineHeight: 1.1 }}>
            {RUGBY_ORG.clubShort} <span style={{ color: T.text3, fontWeight: 400 }}>vs</span> {f.opp}
          </h1>
          <div style={{ display: 'flex', gap: 18, marginTop: 12, fontSize: 12, color: T.text2, flexWrap: 'wrap' }}>
            ...
            <div><span style={{ color: T.text3 }}>Venue</span> <span style={{ color: T.text, marginLeft: 6 }}>The Grange</span></div>
            <div><span style={{ color: T.text3 }}>Squad</span> <span style={{ color: T.good, marginLeft: 6, fontWeight: 600 }}>23 named</span></div>
            <div><span style={{ color: T.text3 }}>Form</span> <span style={{ color: T.text, marginLeft: 6, fontFamily: FONT_MONO }}>W W L W D</span></div>
          </div>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4, color: T.text2, fontSize: 14 }}>
          <div className="tnum" style={{ color: T.text, fontSize: 21, fontWeight: 600, letterSpacing: '-0.01em' }}>{RUGBY_ORG.date}</div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 14 }}><Icon name="cloud" size={15} stroke={1.5} /> 11° · light rain</div>
          <div className="tnum" style={{ display: 'flex', alignItems: 'center', gap: 6, color: T.warn, fontSize: 14 }}>
            <Icon name="cloud" size={15} stroke={1.5} /> 14 mph SW
          </div>
...
export function StatTiles({ T, accent, density }: Common) {
  const tiles = RUGBY_TOP_STATS.map(s => ({
...
function InboxRow({ T, c, first }: { T: ThemeTokens; c: RugbyInboxChannel; first: boolean }) {
  const [h, setH] = useState(false)
  return (
...
export function Squad({ T, accent, density }: Common) {
  const all = [...RUGBY_STARTERS, ...RUGBY_BENCH]
  const out = all.filter(p => p.status === 'out').length
  const doubt = all.filter(p => p.status === 'doubt').length
  const fit = 23 - out - doubt
...
function FixtureCard({ T, accent, f, hot, onClick }: { T: ThemeTokens; accent: AccentTokens; f: RugbyFixture; hot: boolean; onClick?: () => void }) {
  const [h, setH] = useState(false)
  return (
    <div
      onClick={onClick}
...
export function Fixtures({ T, accent, density, onPick }: Common & { onPick?: (f: RugbyFixture) => void }) {
  return (
    <Card T={T} density={density} style={{ gridColumn: '1 / span 7' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10 }}>
        {RUGBY_FIXTURES.slice(0, 4).map((f, i) => <FixtureCard key={i} T={T} accent={accent} f={f} hot={i === 0} onClick={() => onPick?.(f)} />)}
      </div>
    </Card>
  )
}
...
export function Recents({ T, density }: Common) {
  return (
    <Card T={T} density={density} style={{ gridColumn: '1 / span 7' }}>
      <SectionHead T={T} title="Recent results · 1st XV" right={
        <div style={{ display: 'flex', gap: 4 }}>
          {RUGBY_RECENTS.map((m, i) => {
    ...
    <Card T={T} density={density} style={{ gridColumn: '8 / span 5', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 14 }}>
        ...
        <span className="tnum" style={{ fontSize: 36, fontWeight: 500, letterSpacing: '-0.03em', color: accent.hex, fontFamily: FONT }}>
          {s.position}<span style={{ fontSize: 14, color: T.text3 }}>{posSuffix}</span>
        </span>
        <span style={{ fontSize: 10.5, color: T.good, fontWeight: 600 }}>↑1</span>
        <span className="tnum" style={{ fontSize: 11, color: T.text3, marginLeft: 'auto', fontFamily: FONT_MONO }}>52 pts</span>
      </div>
      <div style={{ display: 'flex', gap: 3, marginBottom: 8 }}>
        {RUGBY_SEASON_FORM.map((r, i) => {
          const t = resTint(T, r)
          return (
            <div key={i} style={{
