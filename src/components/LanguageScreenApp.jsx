...
function Note({ color, bg, text }) {
  return (
    <div style={{ background:bg, borderBottom:`1px solid ${color}28`, padding:"8px 22px", flexShrink:0 }}>
      <p style={{ margin:0, fontSize:12.5, color:color, fontWeight:500, fontFamily:F.body, lineHeight:1.55 }}>
  ...
  return (
    <div style={{ flex:1,display:"flex",flexDirection:"column",minHeight:0,background:C.surface }}>
      ...
      <div style={{ flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:28 }}>
        {/* Picture */}
        <div key={idx} className={`nls-pop ${flash==="g"?"nls-flash-g":flash==="r"?"nls-flash-r":""}`} style={{
          width:220,height:220,borderRadius:28,background:C.white,
          border:`3px solid ${C.border}`,boxShadow:"0 12px 40px rgba(0,0,0,.12)",
          display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",
          marginBottom:22,position:"relative",
        }}>
          <div style={{ fontSize:96 }}>{item.emoji}</div>
          <div style={{ position:"absolute",top:10,right:14,fontSize:12,fontWeight:700,color:C.slate,fontFamily:F.body }}>#{idx+1}</div>
        </div>
        {/* Answer hint */}
        <div style={{ background:C.navy,color:"#94a3b8",borderRadius:12,padding:"8px 20px",marginBottom:28,fontSize:12.5,fontFamily:F.body,display:"flex",gap:10,alignItems:"center" }}>
