...
// src/app/womens/[slug]/_components/TripCostCompare.tsx
//
// Cost-compare tab — the same-day-vs-overnight comparison, now a SUPPORTING
// view inside Travel & Logistics (the AI Travel Researcher is the flagship).
// Surfaces both costs side by side + the difference + a non-binding
// recommendation, and keeps the per-trip "draft enquiry → simulated send"
// flow. All numbers come from the shared decision engine; rates are
// placeholders (trip-rates.ts).

import { useEffect, useState } from 'react'
import {
  Bus, Clock, MapPin, Users, ArrowRight, Plus, X,
  Mail, Sparkles, AlertTriangle, CheckCircle2, Moon, Sun,
} from 'lucide-react'
import type { TripDataSource } from '@/lib/sports/travel/trip-data-source'
import type {
...
const hrs = (min?: number | null) => (min == null ? '—' : (min / 60).toFixed(1) + 'h')
...
export default function TripCostCompare({ ds, mode }: { ds: TripDataSource; mode: DataMode }) {
    ...
    <div className="space-y-4">
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <p className="text-sm text-gray-400 max-w-xl">
          Every away trip costed both ways — <span className="text-amber-400">same-day</span> vs{' '}
          <span className="text-indigo-300">overnight</span> — so the saving (or the late-night cost) is
          always in front of you. The call stays yours.
        </p>
        <button onClick={() => setShowCreate(true)} className="inline-flex items-center gap-1.5 rounded-lg px-3 py-2 text-sm font-semibold text-white" style={{ backgroundColor: ACCENT }}>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-6">
          <div className="space-y-3">
            <div className="text-xs font-semibold uppercase tracking-wide text-gray-500">Away trips ({trips.length})</div>
            {trips.map((t) => (
              <TripCard key={t.id} trip={t} comparison={ds.compare(t)} active={t.id === selectedId} onClick={() => setSelectedId(t.id)} />
            ))}
          </div>
          <div>
            {selected
...
function TripCard({ trip, comparison, active, onClick }: { trip: Trip; comparison: TripCostComparison; active: boolean; onClick: () => void }) {
  return (
    <button onClick={onClick} className={`w-full text-left rounded-xl border p-3 transition ${active ? 'border-blue-600/50 bg-blue-600/10' : 'border-gray-800 bg-[#0D1117] hover:border-gray-700'}`}>
...
function TripDetail({ ds, trip, suppliers, mode }: { ds: TripDataSource; trip: Trip; suppliers: TripSupplier[]; mode: DataMode }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <CostColumn title="Same-day (day trip)" icon={<Sun className="w-4 h-4 text-amber-400" />} breakdown={cmp.dayTrip} highlight={cmp.recommendation === 'day_trip'} />
        <CostColumn title="Overnight" icon={<Moon className="w-4 h-4 text-indigo-300" />} breakdown={cmp.overnight} highlight={cmp.recommendation === 'overnight'}
      ...
      <div className="rounded-xl border border-gray-800 bg-[#0D1117] p-4">
        ...
        <div className="mt-3 flex items-start gap-2 rounded-lg bg-black/30 p-3">
          {cmp.recommendation === 'overnight' ? <Moon className="w-4 h-4 mt-0.5 shrink-0 text-indigo-300" />
            : cmp.recommendation === 'day_trip' ? <Sun className="w-4 h-4 mt-0.5 shrink-0 text-amber-400" />
              : <AlertTriangle className="w-4 h-4 mt-0.5 shrink-0 text-gray-400" />}
          <div>
            <div className="text-sm font-semibold text-white">
              {cmp.recommendation === 'overnight' ? 'Lumio suggests: overnight' : cmp.recommendation === 'day_trip' ? 'Lumio suggests: day trip' : 'Lumio suggests: your call'}
              {cmp.lateReturn && <span className="ml-2 rounded bg-red-500/15 px-1.5 py-0.5 text-[10px] font-semibold text-red-300">late return {cmp.estimatedReturnAt ? '~' + fmtTime(cmp.estimatedReturnAt) : ''}</span>}
            </div>
            <p className="mt-1 text-xs leading-snug text-gray-400">{cmp.rationale}</p>
            <p className="mt-1 text-[11px] text-gray-600">Figures use placeholder rates — both numbers always shown so the decision stays yours.</p>
          </div>
        </div>
      </div>
...
function CostColumn({ title, subtitle, icon, breakdown, highlight }: { title: string; subtitle?: string; icon: React.ReactNode; breakdown: TripCostComparison['dayTrip']; highlight?: boolean }) {
  return (
    <div className={`rounded-xl border p-4 ${highlight ? 'border-blue-600/40 bg-blue-600/5' : 'border-gray-800 bg-[#0D1117]'}`}>
      ...
      <div className="mt-3 space-y-1.5">
        ...
        {breakdown.lines.map((l, i) => (
          <div key={i} className="flex items-start justify-between gap-2 text-xs">
            <div className="text-gray-400">{l.label}{l.detail && <span className="block text-[10px] text-gray-600">{l.detail}</span>}</div>
  ...
  return (
    <div className="rounded-xl border border-gray-800 bg-[#0D1117] p-4">
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        {types.map((t) => (
          <button key={t} onClick={() => openDraft(t)} disabled={busy} className="rounded-lg border border-gray-700 px-2.5 py-1.5 text-xs text-gray-200 hover:border-blue-600/50 hover:text-white disabled:opacity-50">{DRAFT_LABELS[t]}</button>
        ))}
      </div>
      {sent && (
        <div className="mt-3 flex items-start gap-2 rounded-lg border border-green-600/30 bg-green-600/10 px-3 py-2 text-xs text-green-200">
          <CheckCircle2 className="w-4 h-4 mt-0.5 shrink-0" /><span>{sent}</span>
        </div>
      )}
      {draft && (
        <div className="mt-4 rounded-lg border border-gray-800 bg-black/30 p-3">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-gray-300">{DRAFT_LABELS[draft.type]} {draft.isCanned && <span className="ml-1 rounded bg-gray-800 px-1.5 py-0.5 text-[10px] text-gray-400">canned demo draft</span>}</span>
            <button onClick={() => setDraft(null)} className="text-gray-500 hover:text-white"><X className="w-4 h-4" /></button>
          </div>
          {draft.to !== null && (
            <div className="mt-2 flex items-center gap-1.5 text-[11px] text-gray-500"><Mail className="w-3.5 h-3.5" /> To: {draft.to} {draft.supplierName ? `(${draft.supplierName})` : ''}</div>
          )}
          <label className="mt-3 block text-[11px] text-gray-500">Subject</label>
          <input value={subject} onChange={(e) => setSubject(e.target.value)} className="mt-1 w-full rounded-md border border-gray-700 bg-[#0D1117] px-2.5 py-1.5 text-sm text-white outline-none focus:border-blue-600/60" />
          <label className="mt-3 block text-[11px] text-gray-500">Message (editable)</label>
          <textarea value={body} onChange={(e) => setBody(e.target.value)} rows={12} className="mt-1 w-full rounded-md border border-gray-700 bg-[#0D1117] px-2.5 py-2 text-xs leading-relaxed text-gray-200 outline-none focus:border-blue-600/60 font-mono" />
          <div className="mt-3 flex items-center justify-end gap-2">
            ...
            <button onClick={send} disabled={busy} className="inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-semibold text-white disabled:opacity-50" style={{ backgroundColor: ACCENT }}>
...
function CreateTripModal({ onClose, onCreate }: { onClose: () => void; onCreate: (input: import('@/lib/sports/travel/trip-types').NewTripInput) => Promise<void> }) {
  ...
  const field = 'w-full rounded-md border border-gray-700 bg-[#0D1117] px-2.5 py-1.5 text-sm text-white outline-none focus:border-blue-600/60'
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={onClose}>
      <div className="w-full max-w-lg rounded-xl border border-gray-800 bg-[#0D1117] p-5 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mt-4 grid grid-cols-2 gap-3">
          <div className="col-span-2"><label className={lbl}>Title *</label><input className={field} value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g. Riverton Albion (A)" /></div>
          ...
          <div className="col-span-2 flex items-center gap-2">
            <input id="ov2" type="checkbox" checked={overnight} onChange={(e) => setOvernight(e.target.checked)} className="accent-blue-600" />
            <label htmlFor="ov2" className="text-sm text-gray-300">Plan as overnight</label>
            {overnight && (
        ...
        <p className="mt-3 text-[11px] text-gray-600">Lumio shows both the same-day and overnight cost the moment you save — the plan toggle is just your starting assumption, not a locked choice.</p>
