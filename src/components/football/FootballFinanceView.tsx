...
// Men's Club Finance — CEO/owner financial command centre.
...
// ─── Revenue (YTD, £k) ─────────────────────────────────────────────────────
const REVENUE = [
  { source: 'Broadcasting',             amount: 9000,  trend: '+4%',  color: C.blue },
  { source: 'Commercial & sponsorship', amount: 11000, trend: '+12%', color: C.warn },
  { source: 'Matchday',                 amount: 9000,  trend: '+8%',  color: C.good },
  { source: 'EFL central distribution', amount: 5000,  trend: '—',    color: C.purple },
  { source: 'Retail & merchandise',     amount: 3000,  trend: '+22%', color: C.cyan },
  { source: 'Other',                    amount: 1000,  trend: '+4%',  color: C.muted },
]
const REVENUE_TOTAL = REVENUE.reduce((s, r) => s + r.amount, 0) // 38000

// ─── Costs (YTD, £k) ───────────────────────────────────────────────────────
const COSTS = [
  { source: 'Player & staff wages',   amount: 24000, color: C.primary },
  { source: 'Transfer amortisation',  amount: 5000,  color: C.gold },
  { source: 'Football operations',    amount: 3500,  color: C.blue },
  { source: 'Facilities & matchday',  amount: 2500,  color: C.good },
  { source: 'Academy & youth',        amount: 2000,  color: C.purple },
  { source: 'Admin & overheads',      amount: 1000,  color: C.cyan },
  { source: 'Travel & logistics',     amount: 500,   color: C.teal },
  { source: 'Medical & performance',  amount: 300,   color: C.warn },
  { source: 'Commercial & marketing', amount: 200,   color: C.muted },
]
const COSTS_TOTAL = COSTS.reduce((s, c) => s + c.amount, 0) // 39000
const OWNER_FUNDING = 3000
const NET_OPERATING = REVENUE_TOTAL - COSTS_TOTAL // -1000
const NET_RESULT = NET_OPERATING + OWNER_FUNDING   // +2000

// ─── Monthly trend (£k) ────────────────────────────────────────────────────
const REV_TREND  = [2900, 2850, 3100, 3300, 3200, 3600, 3000, 3050, 3200, 3350, 3250, 3200]
const COST_TREND = [3200, 3250, 3300, 3350, 3300, 3400, 3450, 3250, 3300, 3350, 3300, 3350]

// ─── Wages ─────────────────────────────────────────────────────────────────
const WAGE_BY_GROUP = [
  { group: 'Forwards',    amount: 7800, color: C.primary },
  { group: 'Midfield',    amount: 6800, color: C.purple },
  { group: 'Defence',     amount: 6200, color: C.blue },
  { group: 'Goalkeepers', amount: 1600, color: C.good },
  { group: 'Coaching & staff', amount: 1600, color: C.muted },
]
const TOP_EARNERS = [
  { player: 'Dean Morris',  pos: 'LW', weekly: 48000 },
  { player: 'Sam Porter',   pos: 'ST', weekly: 40000 },
  { player: 'Chris Nwosu',  pos: 'ST', weekly: 36000 },
  { player: 'Liam Barker',  pos: 'CM', weekly: 34000 },
  { player: 'Daniel Webb',  pos: 'CB', weekly: 32000 },
  { player: 'Jordan Hayes', pos: 'GK', weekly: 30000 },
]
const WAGE_BILL_TREND = [21000, 21500, 22000, 22500, 23000, 23200, 23400, 23600, 23800, 24000, 24000, 24000]
...
const CONTRACTS: Contract[] = [
  { player: 'Sam Porter',   pos: 'ST',  weekly: 40000, end: 'Jun 2026', status: 'Offered',     agent: 'KH Group',         priority: 'High' },
  { player: 'Connor Walsh',  pos: 'CM', weekly: 24000, end: 'Jun 2026', status: 'Negotiating', agent: 'Apex Sports',      priority: 'High' },
  { player: 'Chris Nwosu',  pos: 'ST',  weekly: 36000, end: 'Jun 2026', status: 'Negotiating', agent: 'Athletes United',  priority: 'High' },
  { player: 'Kyle Osei',    pos: 'RB',  weekly: 28000, end: 'Jun 2026', status: 'Standard',    agent: 'Apex Sports',      priority: 'Medium' },
  { player: 'Marcus Reid',  pos: 'CB',  weekly: 24000, end: 'Jun 2026', status: 'Standard',    agent: 'Self-represented', priority: 'Medium' },
  { player: 'Dean Morris',  pos: 'LW',  weekly: 48000, end: 'Jun 2028', status: 'Standard',    agent: 'KH Group',         priority: 'Low' },
  { player: 'Liam Barker',  pos: 'CM',  weekly: 34000, end: 'Jun 2027', status: 'Standard',    agent: 'Apex Sports',      priority: 'Low' },
  { player: 'Daniel Webb',  pos: 'CB',  weekly: 32000, end: 'Jun 2027', status: 'Standard',    agent: 'Self-represented', priority: 'Low' },
  { player: 'Jordan Hayes', pos: 'GK',  weekly: 30000, end: 'Jun 2027', status: 'Standard',    agent: 'KH Group',         priority: 'Low' },
]

// ─── Budgets & Transfers (£k) ──────────────────────────────────────────────
const TRANSFER_BUDGET = 8000
const TRANSFER_COMMITTED = 4800
const TRANSFER_AVAILABLE = TRANSFER_BUDGET - TRANSFER_COMMITTED
const TRANSFER_ACTIVITY = [
  { dir: 'in',  player: 'Antoine Rowe',  detail: 'Forward · from Meridian City',   fee: 2200 },
  { dir: 'in',  player: 'Marco Bianchi',  detail: 'Centre mid · from Crown Park',   fee: 1600 },
  { dir: 'in',  player: 'Isaac Kemp',     detail: 'CB · from Northgate City',       fee: 1000 },
  { dir: 'out', player: 'Owen Hartley',   detail: 'Winger · to Crown Park Galaxy',  fee: 1400 },
]
const BUDGET_ALLOCATION = [
  { dept: 'First-team transfers', amount: 8000, color: C.primary },
  { dept: 'Wages headroom',       amount: 3000, color: C.blue },
  { dept: 'Academy investment',   amount: 2000, color: C.purple },
  { dept: 'Facilities & pitch',   amount: 1500, color: C.good },
  { dept: 'Performance / medical', amount: 800, color: C.warn },
]
...
const SPONSORS = [
  { name: 'Meridian Watches',     deal: 'Front-of-shirt (principal)', value: 420, end: '2026', color: C.primary },
  { name: 'Apex Performance',     deal: 'Kit + technical',            value: 185, end: '2028', color: C.blue },
  { name: 'Northshore Brewing',   deal: 'Stadium naming',             value: 95,  end: '2026', color: C.purple },
  { name: 'Vanta Sports Drinks',  deal: 'Hydration partner',          value: 85,  end: '2027', color: C.good },
  { name: 'Riverside Healthcare', deal: 'Medical partner',            value: 62,  end: '2027', color: C.warn },
]

// ─── PSR / financial health ────────────────────────────────────────────────
const FSR_METRICS = [
  { label: 'Wage-to-turnover', value: 63, threshold: 70, unit: '%', good: 'below', note: 'PL / UEFA squad-cost guidance ceiling ~70% — headroom maintained.' },
  { label: 'Rolling 3yr PSR', value: 68, threshold: 105, unit: '£m', good: 'below', note: 'Cumulative losses within the £105m PSR limit.' },
  { label: 'Liquidity (months cover)', value: 3.8, threshold: 3, unit: 'mo', good: 'above', note: 'Reserves cover ~4 months of outgoings.' },
  { label: 'Self-generated revenue', value: 92, threshold: 70, unit: '%', good: 'above', note: 'Share of revenue not from owner funding.' },
]
const RISK_FLAGS = [
  { tone: C.warn, text: '3 key contracts (Porter, Walsh, Nwosu) expire Jun 2026 — renew or risk free transfers.' },
  { tone: C.good, text: 'Wage-to-revenue 63% — inside PL / UEFA squad-cost guidance.' },
  { tone: C.good, text: 'Small operating deficit before owner funding; net +£2m after funding.' },
  { tone: C.good, text: 'PSR 3-year headroom ~£37m remaining over the rolling window.' },
]
...
export default function FootballFinanceView() {
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm mt-1" style={{ color: C.textSec }}>The owner&apos;s financial command centre — revenue, costs, wages, contracts, transfers, cash flow and PSR health. For multi-horizon planning see Financial Planning under COMMERCIAL.</p>
      </div>
      ...
      {tab === 'overview' && (
        <div className="space-y-4">
          <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3">
            ...
            <StatCard label="Wage / revenue" value={`${wageRev}%`} sub="guidance 70%" tone={C.good} icon={BarChart3} color={C.warn} />
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <Panel title="Profit & Loss summary">
              ...
              <p className="text-[10px] mt-3" style={{ color: C.muted }}>Small operating deficit before owner funding; net +£2m after funding. Within PSR limits.</p>
            </Panel>
          </div>
      ...
      {tab === 'revenue' && (
        <div className="space-y-4">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            <StatCard label="Revenue (YTD)" value={fmtK(REVENUE_TOTAL)} sub="+16% YoY" tone={C.good} icon={TrendingUp} color={C.blue} />
            <StatCard label="Top stream" value="Commercial" sub={fmtK(11000)} icon={BarChart3} color={C.blue} />
            <StatCard label="Fastest grower" value="Retail" sub="+22% YoY" tone={C.good} icon={TrendingUp} color={C.cyan} />
            <StatCard label="Commercial / sponsor" value={fmtK(11000)} sub="29% of revenue" icon={DollarSign} color={C.warn} />
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      ...
      {tab === 'costs' && (
        <div className="space-y-4">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <StatCard label="Academy investment" value={fmtK(2000)} sub="youth pathway" icon={TrendingUp} color={C.purple} />
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      ...
      {tab === 'wages' && (
        <div className="space-y-4">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <StatCard label="Wage / revenue" value={`${wageRev}%`} sub="guidance 70%" tone={C.good} icon={BarChart3} color={C.warn} />
            <StatCard label="Highest earner" value="D. Morris" sub="£48,000/wk" icon={DollarSign} color={C.primary} />
            <StatCard label="Avg first-team wage" value="£22,000/wk" sub="across 21 squad" icon={TrendingUp} color={C.blue} />
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      ...
      {tab === 'fsr' && (
        <div className="space-y-4">
          <div className="rounded-xl p-4" style={{ background: `linear-gradient(135deg, ${C.good}14, ${C.cardAlt})`, border: `1px solid ${C.good}40` }}>
            ...
            <p className="text-xs mt-1" style={{ color: C.textSec }}>Within EFL Profit &amp; Sustainability (PSR) and the incoming UEFA squad-cost thresholds on all monitored metrics.</p>
          </div>
          ...
          <p className="text-[10px]" style={{ color: C.muted }}>Demo — figures illustrative. PSR thresholds shown are indicative; a signed client would sync to filed accounts and league monitoring submissions.</p>
        </div>
      )}
    </div>
  )
