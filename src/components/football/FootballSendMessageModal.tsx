...
// Football Pro portal — Send Message composer (mirrors the Women's composer).
//
...
//
// Blue-themed (Football Pro accent). DEMO-SAFE: no real messages sent.
...
const C = {
  ...
  pinkDeep:   '#003DA5',
  pinkLight:  '#60A5FA',
  pinkDim:    'rgba(0,61,165,0.15)',
  pinkBorder: 'rgba(37,99,235,0.5)',
  good:       '#22C55E',
  danger:     '#EF4444',
}

const TEAM = [
  { name: 'Tom Reynolds',    role: 'Manager',              icon: '🎽' },
  { name: 'Dr Mark Ellis',   role: 'Club Doctor',          icon: '🏥' },
  { name: 'Sam Doyle',       role: 'Head of Performance',  icon: '🏃' },
  { name: 'James Hart',      role: 'Performance Analyst',  icon: '📊' },
  { name: 'Paul Stevens',    role: 'Director of Football', icon: '📋' },
  { name: 'James Hartley',   role: 'Chairman',             icon: '🏛️' },
  { name: 'Lee Walters',     role: 'Head Physio',          icon: '🩺' },
  { name: 'Mark Foster',     role: 'Commercial Director',  icon: '💼' },
]
...
const CANNED_STANDARD = `Hi all — quick coordination ahead of Saturday vs Hartwell Town.

Squad announcement Friday morning. Recovery + video review as normal Thursday. Set-piece work Friday PM in the analysis room. Travel and away-end details confirmed.

Thanks — Tom`

const CANNED_URGENT = `[URGENT] Henderson hamstring flagged in training today — scan booked 14:00.

Need Sam and Dr Ellis in the office in the next 30 minutes to confirm a modified Saturday plan. Squad announcement may shift to Saturday AM. Reply when received.

Tom`

type Step = 'who' | 'how' | 'message' | 'preview' | 'sent'

export default function FootballSendMessageModal({ onClose }: { onClose: () => void }) {
