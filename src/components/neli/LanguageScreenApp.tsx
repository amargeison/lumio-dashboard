'use client';
/**
 * LanguageScreenApp.tsx (converted from .jsx)
 *
...
/* ─── Celebration burst ───────────────────────────────────────────────────── */
function CelebrationBurst({ show }: any) {
  if (!show) return null;
  return (
...
function Pips({ total, results }: any) {
  return (
...
function SubtestHeader({ title, icon, color, results, total }: any) {
...
function Note({ color, bg, text }: any) {
  return (
...
function ScoreBtns({ onCorrect, onIncorrect, disabled }: any) {
  return (
...
/* ─── Owl Intro Sequence ──────────────────────────────────────────────────── */
function OwlIntro({ onDismiss }: any) {
...
/* ─── Screen 2: Setup ───────────────────────────────────────────────────────── */
function Setup({ prefill, onConfirm, onBack }: any) {
  ...
  const set = (k: any, v: any) => { setForm((f: any)=>({...f,[k]:v})); setErrs((e: any)=>({...e,[k]:null})); };
...
function EVSubtest({ results, onScore, voiceOn }: any) {
  ...
  useEffect(()=>{
    function h(e: any){ if(e.key==="Enter")doScore(true); if(e.key==="Backspace")doScore(false); }
...
function RVSubtest({ results, onScore, voiceOn }: any) {
  const idx = results.length;
  ...
  const doScore = useCallback((selIdx: any) => {
    if (busy||!item) return;
    setBusy(true);
...
function SRSubtest({ results, onScore, voiceOn }: any) {
  const idx = results.length;
...
function NarrSubtest({ results, onScore, voiceOn }: any) {
...
/* ─── Screen: Results ───────────────────────────────────────────────────────── */
function Results({ child, ageYears, subtests, onClose, onComplete }: any) {
  const totalRaw = useMemo(()=>subtests.reduce((s: any,t: any)=>s+t.results.filter((r: any)=>r.correct).length,0), [subtests]);
  ...
  const subtestScores = useMemo(()=>subtests.map((s: any)=>{
    const raw=s.results.filter((r: any)=>r.correct).length;
          ...
          <div style={{ display:"flex",gap:0,borderBottom:`1px solid ${C.border}` }}>
            ...
            <div style={{ flex:2,display:"grid",gridTemplateColumns:"1fr 1fr",padding:"20px 26px",gap:"14px 24px" }}>
              {[
                ...
                { label:"Raw Score", value:`${totalRaw}/${subtests.reduce((s: any,t: any)=>s+t.results.length,0)}` },
...
export default function LanguageScreenApp({
  ...
  function handleConfirm(childData: any) {
    setChild(childData);
  ...
  function handleScore(correct: any) {
    setSubtests((prev: any) => {
      const next = prev.map((t: any,i: any) => {
  ...
  const wrap: any = { position:"fixed",inset:0,zIndex:9999,display:"flex",flexDirection:"column",overflowY:"auto" };
