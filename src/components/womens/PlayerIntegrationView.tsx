...
import {
  ...
  Truck, Scroll, Heart, Activity, ShieldCheck,
} from 'lucide-react'

// Women's — Player Integration. Top-class onboarding & integration module for
// international / relocating players. Live draggable welfare board + recent
// activity, expandable per-player stage checklist, and per-area tabs. Demo only.
...
const C = {
  ...
  pink: '#EC4899', pinkDeep: '#BE185D', good: '#22C55E', amber: '#F59E0B', red: '#EF4444',
}
const rag = (s: string): { background: string; color: string } =>
  /complete|done|approved|registered|granted|valid|settled|signed|active|low|fluent|native|received|opened|confirmed|connected|up to date|drives|held|linked|reviewed|translated/i.test(s) ? { background: 'rgba(34,197,94,0.18)', color: '#4ADE80' } :
...
const isDone = (s: string) => /complete|approved|registered|granted|valid|settled|signed|active|fluent|native|intermediate|received|opened|confirmed|connected|up to date|drives|held|linked|reviewed|translated|low|none/i.test(s)
...
type P = {
  ...
  welfare: { lastCheckin: string; homesickness: string; pfa: string; concerns: string; safeguarding: string }
}
const PLAYERS: P[] = [
  { id: 'diallo', name: 'Amara Diallo', flag: '🇸🇳', nat: 'Senegal', pos: 'FW', arrival: 'Jul 2025', pct: 92, buddy: 'Abbi Walsh',
    imm: { gbe: 'Approved', visa: 'ISP granted', visaExp: 'Jul 2028', itc: 'Received', permit: 'Up to date' },
    setup: { housing: 'Lease signed · moved in', banking: 'Account opened', ni: 'Received', tax: '1257L confirmed', gp: 'Registered', phone: 'Active', driving: 'UK licence held', transport: 'Drives · Bay 12' },
    family: { partner: 'Settled', children: '2 · school places secured', schools: 'Riverside Primary (EAL)', childcare: 'Arranged' },
    lang: { level: 'Intermediate', provider: 'ESOL · Surrey College', interpreter: 'On call', liaison: 'Ousmane Ba (community)', community: 'Connected' },
    welfare: { lastCheckin: '3 days ago', homesickness: 'Low', pfa: 'Shared at signing', concerns: 'None', safeguarding: 'Up to date' } },
  { id: 'fatima', name: 'Fatima Al-Said', flag: '🇪🇬', nat: 'Egypt', pos: 'AM', arrival: 'Aug 2025', pct: 84, buddy: 'Priya Nair',
    imm: { gbe: 'Approved', visa: 'ISP granted', visaExp: 'Aug 2028', itc: 'Pending', permit: 'Up to date' },
    ...
    family: { partner: 'N/A', children: 'N/A', schools: 'N/A', childcare: 'N/A' },
    lang: { level: 'Elementary', provider: 'ESOL · Surrey College', interpreter: 'Agency (Arabic)', liaison: 'Assigned', community: 'Connected' },
    welfare: { lastCheckin: 'This week', homesickness: 'Medium', pfa: 'Shared at signing', concerns: 'ITC delay tracked', safeguarding: 'Up to date' } },
  { id: 'johansson', name: 'Freya Johansson', flag: '🇸🇪', nat: 'Sweden', pos: 'CM', arrival: 'Jul 2025', pct: 70, buddy: 'Megan Hughes',
    ...
    lang: { level: 'Fluent', provider: 'N/A', interpreter: 'N/A', liaison: 'Assigned', community: 'Connected' },
    welfare: { lastCheckin: 'Fortnightly', homesickness: 'Low', pfa: 'Shared at signing', concerns: 'None', safeguarding: 'Up to date' } },
  { id: 'gallagher', name: 'Niamh Gallagher', flag: '🇮🇪', nat: 'Ireland', pos: 'CB', arrival: 'Aug 2025', pct: 62, buddy: 'Charlotte Reed',
    imm: { gbe: 'N/A (CTA)', visa: 'Common Travel Area', visaExp: '—', itc: 'Received', permit: 'N/A' },
    setup: { housing: 'Viewing scheduled', banking: 'In progress', ni: 'Applied', tax: 'Pending', gp: 'Registered', phone: 'Active', driving: 'IE licence valid', transport: 'Club shuttle' },
    family: { partner: 'N/A', children: 'N/A', schools: 'N/A', childcare: 'N/A' },
    lang: { level: 'Native', provider: 'N/A', interpreter: 'N/A', liaison: 'Assigned', community: 'Connected' },
    welfare: { lastCheckin: 'This week', homesickness: 'Low', pfa: 'Shared at signing', concerns: 'Housing search', safeguarding: 'Up to date' } },
  { id: 'fernandez', name: 'Ana Sofía Fernández', flag: '🇪🇸', nat: 'Spain', pos: 'RW', arrival: 'Jan 2026', pct: 35, buddy: 'Jade Osei',
    imm: { gbe: 'Approved', visa: 'ISP granted', visaExp: 'Jan 2029', itc: 'Received', permit: 'Up to date' },
    ...
    lang: { level: 'Beginner', provider: 'ESOL · Surrey College', interpreter: 'Agency (Spanish)', liaison: 'Pending match', community: 'In progress' },
    welfare: { lastCheckin: 'This week', homesickness: 'High', pfa: 'Shared at signing', concerns: 'Early-stage settling', safeguarding: 'Up to date' } },
]

const DEADLINES = [
  { date: '15 Apr', who: 'Fatima Al-Said', what: 'ITC registration follow-up with FA', tone: C.amber },
  { date: '22 Apr', who: 'Niamh Gallagher', what: 'House viewing + lease decision', tone: C.amber },
  { date: '28 Apr', who: 'Ana Sofía Fernández', what: 'ESOL course enrolment + interpreter', tone: C.amber },
  { date: '05 May', who: 'Fatima Al-Said', what: 'UK driving theory test', tone: C.text3 },
  { date: '12 May', who: 'Ana Sofía Fernández', what: 'Family relocation — partner job leads', tone: C.text3 },
]
const AREAS = ['Immigration', 'Housing', 'Banking & tax', 'Healthcare', 'Driving', 'Language', 'Family', 'Cultural liaison', 'Wellbeing', 'Buddy / mentor', 'Contract translated', 'Safeguarding']

// ─── Welfare board ──────────────────────────────────────────────────────────
const STAGES: Stage[] = ['NEW ARRIVAL', 'SETTLING IN', 'ESTABLISHED', 'MONITORING']
const INITIAL_STAGE: Record<string, Stage> = {
  fernandez: 'NEW ARRIVAL', fatima: 'SETTLING IN', gallagher: 'SETTLING IN', johansson: 'ESTABLISHED', diallo: 'ESTABLISHED',
}
const ragTone = (pct: number) => pct >= 80 ? C.good : pct >= 50 ? C.amber : C.red

const ACTIVITY = [
  { t: '10:22', txt: 'Fernández: ESOL enrolment confirmed — Surrey College, Mon/Wed AM' },
  { t: '09:48', txt: 'Gallagher: house viewing booked — 2-bed, Kingston, 22 Apr' },
  { t: '09:15', txt: 'Al-Said: ITC follow-up sent to FA — registration chase' },
  { t: 'Yest', txt: 'Fernández: Spanish-speaking cultural liaison match in progress' },
  { t: 'Yest', txt: 'Diallo: visa renewal reminder set — July 2028 window' },
  { t: '2d', txt: 'Al-Said: UK driving theory test booked — 05 May' },
  { t: '2d', txt: 'Fernández: partner relocation — job leads shared, school-place enquiry' },
  { t: '3d', txt: 'Johansson: fortnightly wellbeing check-in — settled, no concerns' },
  { t: '4d', txt: 'Quarterly wellbeing survey closed — 23/24 responses, 8.4/10 satisfaction' },
]

// ─── Stage checklist builder (per player) ───────────────────────────────────
...
const buildStages = (p: P): StageItem[] => [
  ...
  { id: 'driving', title: 'Driving', icon: Car, status: p.setup.driving, details: [
    { label: 'Licence', value: p.setup.driving }, { label: 'UK conversion', value: /held|valid/i.test(p.setup.driving) ? 'Not required' : 'In progress' }, { label: 'Lessons', value: /progress|pending/i.test(p.setup.driving) ? 'Booked' : 'N/A' } ] },
  ...
  { id: 'maternity', title: 'Maternity & Return-to-Play', icon: Heart, status: 'Reviewed', details: [
    { label: 'Status', value: 'No active plan · reviewable on request' }, { label: 'Maternity policy', value: 'Provided (translated)' }, { label: 'Return-to-play protocol', value: 'On file · UEFA-aligned' }, { label: 'Contract clause', value: 'Discussed at signing' } ] },
  { id: 'cycle', title: 'Cycle-Tracking Integration', icon: Activity, status: 'Linked', details: [
    { label: 'Cycle-tracking module', value: 'Linked · player consented' }, { label: 'Training adjustment', value: 'Phased load enabled' }, { label: 'Education', value: 'Provided (translated)' }, { label: 'Privacy', value: 'Player-controlled visibility' } ] },
  { id: 'safeguarding', title: "Women's-Specific Safeguarding", icon: ShieldCheck, status: p.welfare.safeguarding, details: [
    { label: 'Independent welfare officer', value: 'Sarah Martinez' }, { label: 'Women in Football network', value: 'Liaison contacts shared' }, { label: 'Anti-harassment training', value: 'Completed at induction' }, { label: 'Carney Review compliance', value: p.welfare.safeguarding } ] },
]
...
export default function PlayerIntegrationView() {
  ...
  return (
    <div className="space-y-5">
      <div><h2 className="text-xl font-bold flex items-center gap-2" style={{ color: C.text }}><Globe2 size={18} style={{ color: C.pinkDeep }} /> Player Integration</h2><p className="text-sm mt-1" style={{ color: C.text3 }}>End-to-end onboarding for international &amp; relocating players — immigration, relocation, family, language, culture and welfare.</p></div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <Stat label="International players" value={String(PLAYERS.length)} sub="in the squad" color={C.text} />
        <Stat label="Avg integration" value={`${avg}%`} sub="across all areas" color="#F472B6" />
      ...
      <div className="flex gap-1 border-b overflow-x-auto" style={{ borderColor: C.border }}>
        {tabs.map(t => (<button key={t.id} onClick={() => setTab(t.id)} className="px-4 py-2 text-xs font-semibold -mb-px whitespace-nowrap" style={{ borderBottom: `2px solid ${tab === t.id ? C.pinkDeep : 'transparent'}`, color: tab === t.id ? '#F472B6' : C.text4 }}>{t.label}</button>))}
      </div>

      {tab === 'overview' && (
        <div className="space-y-4">
          {/* ── Live draggable welfare board ── */}
          <div>
              ...
              {STAGES.map(s => {
                ...
                return (
                  <div key={s} onDragOver={e => { e.preventDefault(); setOverStage(s) }} onDragLeave={() => setOverStage(o => o === s ? null : o)} onDrop={() => drop(s)}
                    ...
                    style={{ backgroundColor: isOver ? 'rgba(236,72,153,0.06)' : C.panel2, border: `1px solid ${isOver ? C.pink : C.border}` }}>
                      ...
                      return (
                        <div key={p.id} draggable onDragStart={() => setDragId(p.id)} onDragEnd={() => { setDragId(null); setOverStage(null) }}
                          ...
                          <div className="flex items-center gap-2 mb-2">
                            <div className="flex-1 h-1.5 rounded-full" style={{ background: C.borderHi }}><div className="h-1.5 rounded-full" style={{ width: `${p.pct}%`, background: C.pink }} /></div>
          ...
          {/* ── Expandable integration progress ── */}
          <div className="rounded-xl p-5" style={{ backgroundColor: C.panel, border: `1px solid ${C.border}` }}>
            ...
            <div className="space-y-2">{PLAYERS.map(p => {
              ...
              return (
                <div key={p.id} className="rounded-lg overflow-hidden" style={{ border: `1px solid ${open ? C.pinkDeep + '66' : C.border}`, background: open ? C.panelAlt : 'transparent' }}>
                  <button onClick={() => { setOpenPlayer(open ? null : p.id); setOpenStage(null) }} className="w-full text-left px-3 py-2.5">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="flex items-center gap-2"><span className="text-[10px]" style={{ color: C.text4 }}>{doneCount}/{stages.length} stages</span><span className="text-xs font-bold" style={{ color: '#F472B6' }}>{p.pct}%</span><span style={{ color: C.text4, fontSize: 11 }}>{open ? '▾' : '▸'}</span></span>
                    </div>
                        ...
                        return (
                          <div key={st.id} className="rounded-lg" style={{ backgroundColor: done ? `${C.good}0a` : C.panel, border: `1px solid ${done ? C.good + '30' : C.border}` }}>
                            <div className="flex items-center gap-3 p-2.5 cursor-pointer" onClick={() => setOpenStage(so ? null : sk)}>
                              ...
                              <ItemIcon size={14} style={{ color: done ? C.good : C.pink }} className="flex-shrink-0" />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div className="rounded-xl p-5" style={{ backgroundColor: C.panel, border: `1px solid ${C.border}` }}>
              ...
              {DEADLINES.map((d, i) => (<div key={i} className="flex items-start gap-3 py-2" style={{ borderBottom: i < DEADLINES.length - 1 ? `1px solid ${C.border}80` : undefined }}><span className="text-[11px] font-bold w-12 shrink-0" style={{ color: C.pinkDeep === d.tone ? '#F472B6' : d.tone }}>{d.date}</span><div><div className="text-xs font-medium" style={{ color: C.text2 }}>{d.who}</div><div className="text-[11px]" style={{ color: C.text4 }}>{d.what}</div></div></div>))}
            </div>
            <div className="rounded-xl p-5" style={{ backgroundColor: C.panel, border: `1px solid ${C.border}` }}>
      ...
      {tab === 'immigration' && (
        <div className="space-y-3">
          ...
          <div className="rounded-xl p-4 text-[11px]" style={{ background: 'rgba(190,24,93,0.08)', borderLeft: `3px solid ${C.pinkDeep}`, color: C.text2 }}>
            Workflow: GBE points assessment → ISP visa application → biometrics → FA international clearance (ITC) → registration. Immigration solicitor: M. Carter, Stowe &amp; Hart LLP. Sponsor-licence audit tracked centrally; renewal windows flagged 6 months out.
          </div>
        </div>
      )}
      ...
      {tab === 'setup' && (
        <div className="space-y-3">
          ...
          <div className="rounded-xl p-4 text-[11px]" style={{ background: 'rgba(190,24,93,0.08)', borderLeft: `3px solid ${C.pinkDeep}`, color: C.text2 }}>
            Relocation concierge handles housing search (within 8 miles of training), utilities, bank account (passport + visa + club letter + proof of address), NI number, PAYE/tax code, GP &amp; dental registration, phone &amp; broadband, and UK driving-licence conversion.
          </div>
        </div>
      )}
      ...
      {tab === 'welfare' && (
        <div className="space-y-3">
          <Tbl head={['Player', 'Last check-in', 'Homesickness', 'Buddy / mentor', 'PFA line', 'Open concerns', 'Safeguarding']} rows={PLAYERS.map(p => [
            `${p.flag} ${p.name}`, p.welfare.lastCheckin, { v: p.welfare.homesickness, rag: true }, p.buddy, { v: p.welfare.pfa, rag: true }, p.welfare.concerns, { v: p.welfare.safeguarding, rag: true },
          ])} />
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div className="rounded-xl p-4 text-[11px]" style={{ background: 'rgba(190,24,93,0.08)', borderLeft: `3px solid ${C.pinkDeep}`, color: C.text2 }}>
              Buddy / mentor system pairs each new arrival with a settled team-mate. Wellbeing check-ins (weekly early on, then fortnightly) track homesickness, training adjustment, language confidence and sleep. PFA wellbeing line shared at signing.
            </div>
            <div className="rounded-xl p-4 text-[11px]" style={{ background: 'rgba(190,24,93,0.08)', borderLeft: `3px solid ${C.pinkDeep}`, color: C.text2 }}>
              Women&apos;s-specific: independent welfare officer access, maternity policy &amp; return-to-play pathway (translated), opt-in cycle-tracking education, anti-harassment training at induction and Karen Carney Review safeguarding standards.
            </div>
          </div>
        </div>
      )}
