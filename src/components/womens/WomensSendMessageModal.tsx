...
// Women's portal — Send Message composer.
//
...
//
// Pink-themed (Women's portal accent). DEMO-SAFE: no real messages sent.
...
const C = {
  ...
  pinkDeep:   '#BE185D',
  pinkLight:  '#F472B6',
  pinkDim:    'rgba(190,24,93,0.15)',
  pinkBorder: 'rgba(190,24,93,0.5)',
  good:       '#22C55E',
  danger:     '#EF4444',
}

const TEAM = [
  { name: 'Sarah Frost',     role: 'Head Coach',           icon: '🎽' },
  { name: 'Dr Anna Reid',    role: 'Club Doctor',          icon: '🏥' },
  { name: 'Nina Walsh',      role: 'Welfare Lead',         icon: '❤️' },
  { name: 'Marcus Chen',     role: 'S&C Coach',            icon: '🏃' },
  { name: 'James Kerr',      role: 'Performance Analyst',  icon: '📊' },
  { name: 'Helen Voss',      role: 'Director of Football', icon: '📋' },
  { name: 'Kate Brennan',    role: 'Club Director',        icon: '🏛️' },
  { name: 'Jordan Clarke',   role: 'Commercial Director',  icon: '💼' },
]
...
const CANNED_STANDARD = `Hi all — quick coordination ahead of Saturday vs Hartwell Women.

Squad announcement Friday morning. Welfare check-ins as normal Thursday. Anyone with cycle-phase flags this week, message Nina directly. Final tactical brief Friday PM in the team room.

Thanks — Sarah`

const CANNED_URGENT = `[URGENT] Emily Zhang composite ACL risk hit 98 today — load cap immediate.

Need Marcus and Dr Anna in the office in the next 30 minutes to confirm a modified Saturday plan. Squad announcement may shift to Saturday AM. Reply when received.

Sarah`

type Step = 'who' | 'how' | 'message' | 'preview' | 'sent'

export default function WomensSendMessageModal({ onClose }: { onClose: () => void }) {
