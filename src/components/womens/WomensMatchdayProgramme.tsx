...
// Oakridge Women FC vs Hartwell Women · Sat 03 May 2026 · WSL 2.
// ─────────────────────────────────────────────────────────────────────────
...
const ISSUE = { no: 19, price: '£3', date: 'Sat 03 May 2026', ko: '14:00', comp: 'Barclays WSL 2 · MD-19', venue: 'Oakridge Stadium' }
...
export default function WomensMatchdayProgramme({ accent = '#BE185D' }: { accent?: string }) {
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <div className="text-[11.5px] mt-1" style={{ color: C.text3 }}>
              Issue #{ISSUE.no} · Oakridge Women FC vs Hartwell Women · {ISSUE.date} · {ISSUE.comp}
            </div>
          </div>
          <div className="flex items-center gap-2">
            {!built ? (
...
function FlipBook({ accent, sections, onClose, onShare }: { accent: string; sections: Section[]; onClose: () => void; onShare: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[120] flex flex-col" style={{ background: 'radial-gradient(140% 100% at 50% 0%, #2a0716 0%, #0a0509 70%)' }}>
...
function PaperPage({ runningHead, folio, accent, dark, children }: { runningHead: string; folio: string; accent: string; dark?: boolean; children: React.ReactNode }) {
  return (
    <div className="flex-1 flex flex-col relative overflow-hidden prog-sans" style={{ background: dark ? '#3E0820' : P.paper, color: dark ? P.cream : P.ink }}>
      ...
      <div className="px-7 py-2.5 flex items-center justify-between text-[8px] uppercase tracking-[0.22em] relative" style={{ borderTop: `1px solid ${dark ? 'rgba(203,162,75,0.25)' : P.line}`, color: dark ? 'rgba(242,233,216,0.55)' : P.muted }}>
        <span>Oakridge Women FC</span><span style={{ color: dark ? P.goldHi : accent }}>{runningHead}</span><span>{folio}</span>
      </div>
    </div>
  )
}
  ...
  return (
    <figure className="relative w-full overflow-hidden" style={{ height: h, border: `1px solid ${P.line}` }}>
      {showImg ? (<>
        ...
        <div className="absolute inset-0" style={{ background: `linear-gradient(135deg, ${P.claret}bb, #2a0716aa)`, mixBlendMode: 'multiply' }} />
      </>) : (<>
        <div className="absolute inset-0" style={{ background: `linear-gradient(135deg, ${P.claret2} 0%, ${P.claret} 45%, #2a0716 100%)` }} />
...
function buildPages(accent: string, sections: Section[]): React.ReactNode[] {
  ...
  // 1 — COVER
  if (on('cover')) P_(
    <div key="cover" className="flex-1 flex flex-col relative overflow-hidden prog-sans" style={{ background: `linear-gradient(165deg, #9A1452 0%, ${P.claret} 40%, #2E0617 100%)`, color: P.cream }}>
      ...
      <div className="absolute top-0 left-0 right-0 overflow-hidden" style={{ height: '44%' }}>
        <img src="/programme/cover-hero.png" alt="" className="w-full h-full object-cover" style={{ objectPosition: 'center 25%' }} onError={(e) => { (e.currentTarget as HTMLImageElement).style.display = 'none' }} />
        <div className="absolute inset-0" style={{ background: 'linear-gradient(180deg, rgba(46,6,23,0.15) 0%, rgba(122,14,62,0.25) 55%, #7A0E3E 100%)' }} />
      </div>
      <Grain opacity={0.09} />
      <div className="relative flex flex-col h-full px-8 py-9">
        <div className="text-center">
          ...
          <div className="flex items-center justify-center gap-3">
            ...
            <div className="prog-serif text-left leading-[0.92]" style={{ fontSize: 20, fontWeight: 800 }}>OAKRIDGE<br />WOMEN FC</div>
          </div>
        </div>
        <div className="flex-1 flex flex-col justify-center text-center">
          ...
          <div className="prog-serif" style={{ fontSize: 44, fontWeight: 900, lineHeight: 0.95, letterSpacing: '-0.01em' }}>Hartwell</div>
        ...
        <div className="flex items-end justify-between">
          ...
          <div className="text-right px-3 py-1.5" style={{ background: P.goldHi, color: '#2E0617' }}>
  ...
  P_(
    <PaperPage key="contents" runningHead="Contents" folio={String(pages.length + 1).padStart(2, '0')} accent={accent}>
      ...
      <div className="mt-5 prog-serif italic text-[12px] leading-relaxed" style={{ color: P.ink2 }}>
        Welcome to Oakridge Stadium. Thank you to our supporters, partners and the matchday team — and to you, for backing the women&apos;s game.
      </div>
    </PaperPage>
  )
  ...
  if (on('welcome')) P_(
    <PaperPage key="welcome" runningHead="Chair’s welcome" folio={String(pages.length + 1).padStart(2, '0')} accent={accent}>
      ...
      <h1 className="prog-serif" style={{ fontSize: 27, fontWeight: 800, color: P.ink, lineHeight: 1.02 }}>A warm welcome to Oakridge Stadium</h1>
      <GoldRule />
      <div className="my-3"><EditorialPhoto label="Kate Ellison — Chair" h={120} src="/programme/chair.png" /></div>
      <DropCap accent={accent} text="Good afternoon and a very warm welcome to Hartwell Women — their players, staff and supporters — for today’s WSL 2 fixture. It has been a season of real progress on and off the pitch: record season-ticket numbers, a thriving academy and a community programme reaching more girls than ever before." />
      ...
      <div className="prog-serif italic text-[13px] mt-3" style={{ color: accent }}>— Kate Ellison, Chair</div>
      ...
      <div className="mt-3 px-3 py-2.5 text-[11px]" style={{ background: `${accent}0e`, borderLeft: `3px solid ${P.goldHi}`, color: P.ink2 }}>
        <b className="prog-serif" style={{ color: P.ink }}>Did you know?</b> Today is Oakridge Women’s 250th league fixture since the club turned fully professional in 2019.
      </div>
    </PaperPage>
  )
    ...
    <PaperPage key="notes" runningHead="Manager’s notes" folio={String(pages.length + 1).padStart(2, '0')} accent={accent}>
      ...
      <div className="flex items-center gap-3 mb-1">
        <div className="w-12 h-12 rounded-full flex-shrink-0 overflow-hidden" style={{ background: `linear-gradient(135deg, ${P.claret2}, ${P.claret})` }}>
          <img src="/programme/manager.png" alt="" className="w-full h-full object-cover" onError={(e) => { (e.currentTarget as HTMLImageElement).style.display = 'none' }} />
        </div>
        <div><div className="prog-serif text-[16px] font-bold" style={{ color: P.ink }}>Sarah Frost</div><div className="text-[9.5px] uppercase tracking-[0.18em]" style={{ color: P.gold }}>First-team Manager</div></div>
      </div>
      <GoldRule />
      <DropCap accent={accent} text="Hartwell are a well-organised side who sit deep and counter, so we will need patience and quality in the final third. The group has trained brilliantly this week — Carter is back available after her scan, and that is a big lift for everyone." />
      <PullQuote accent={accent}>“We know what three points would do for us in this run-in.”</PullQuote>
      <p className="prog-sans text-[12px] leading-relaxed" style={{ color: P.ink2 }}>Get behind the players from the first whistle — your support on the overlaps and at set-pieces genuinely makes a difference. Up the Rosies.</p>
      ...
      <div className="px-3 py-2.5 mb-3 text-[11px]" style={{ border: `1px solid ${P.line}`, background: '#FBF7EE', color: P.ink2 }}>
        <b className="prog-serif" style={{ color: P.ink }}>Key battle:</b> Emma Clarke vs Lena Marsh — whoever wins the second balls in midfield controls the tempo.
      </div>
      ...
      <MiniLegend accent={accent} a="Oakridge" b="Hartwell" />
    </PaperPage>
  )
  ...
  if (on('teamnews')) P_(
    <PaperPage key="teamnews" runningHead="Team news" folio={String(pages.length + 1).padStart(2, '0')} accent={accent}>
      ...
      <GoldRule />
      {[
        ['Available', 'Jess Carter returns after a negative scan — straight back into contention.', P.claret],
        ['Doubt', 'Mia Davies (Grade 1 MCL) is 2–3 weeks away; targeting the cup tie.', P.gold],
        ['Out', 'Okafor continues her concussion return-to-play protocol (Day 3).', '#9A2A2A'],
        ['Milestone', 'Emma Clarke makes her 100th appearance for the club today.', accent],
      ].map((r, i) => (
  ...
  if (on('opponents')) P_(
    <PaperPage key="opponents" runningHead="Today’s opponents" folio={String(pages.length + 1).padStart(2, '0')} accent={accent}>
      ...
      <h1 className="prog-serif" style={{ fontSize: 26, fontWeight: 800, color: P.ink }}>Hartwell Women</h1>
      <GoldRule />
      <div className="my-3"><EditorialPhoto label="Hartwell Women — squad 2025/26" h={120} src="/programme/opponents.jpg" /></div>
      <div className="grid grid-cols-3 gap-2 mb-3">
        {[['Position', '7th'], ['Form', 'L D W L D'], ['Top scorer', 'R. Boyd (9)']].map(([l, v]) => (
          <div key={l} className="px-2 py-2.5 text-center" style={{ border: `1px solid ${P.line}`, background: '#FBF7EE' }}>
            <div className="text-[8px] uppercase tracking-[0.16em]" style={{ color: P.gold }}>{l}</div>
            <div className="prog-serif text-[14px] font-bold mt-0.5" style={{ color: P.ink }}>{v}</div>
          </div>
      ...
      <p className="prog-sans text-[12px] leading-relaxed" style={{ color: P.ink2 }}>Hartwell arrive mid-table and hard to break down, favouring a compact 5-4-1 and quick transitions. Watch for Rachel Boyd in behind and a set-piece threat from the back post. The reverse fixture finished 1-1 in a tight afternoon at Hartwell Park.</p>
      <div className="mt-3 px-3 py-2.5 text-[11px]" style={{ background: `${accent}0e`, borderLeft: `3px solid ${P.goldHi}`, color: P.ink2 }}>
        <b className="prog-serif" style={{ color: P.ink }}>Ones to watch:</b> Rachel Boyd (FW), Lena Marsh (CM), keeper Amy Doyle — strong on crosses, weaker on the ground.
      </div>
      <SubHead accent={accent}>Head-to-head record</SubHead>
      <StatStrip accent={accent} items={[['10', 'Played'], ['5', 'Oakridge'], ['3', 'Drawn'], ['2', 'Hartwell']]} />
  ...
  if (on('teams')) {
    const home = ['1 Hughes (GK)', '2 Ward', '4 Bennett', '5 Stone (C)', '3 Reed', '6 Nair', '8 Osei', '10 Clarke', '7 Whitmore', '11 Zhang', '9 Carter']
    const away = ['1 Doyle (GK)', '2 Page', '5 Cole', '6 Briggs', '3 Hunt', '4 Marsh', '8 Webb', '7 Dunne', '11 Shaw', '10 Boyd (C)', '9 Friel']
    P_(
      <PaperPage key="teams" runningHead="Today’s teams" folio={String(pages.length + 1).padStart(2, '0')} accent={accent} dark>
        ...
        <div className="grid grid-cols-2 gap-5">
          {[['Oakridge', home, P.goldHi], ['Hartwell', away, 'rgba(242,233,216,0.7)']].map(([name, list, col]) => (
        ...
        <div className="grid grid-cols-2 gap-5 mt-3 text-[9.5px]" style={{ color: 'rgba(242,233,216,0.6)' }}>
          <div><b style={{ color: P.cream }}>Subs:</b> Pike, Lowe, Adeyemi, Quinn, Reid, Tilley, Brennan</div>
          <div><b style={{ color: P.cream }}>Subs:</b> Knox, Pratt, Ellis, Vance, Day, Holt, Ng</div>
        </div>
        <div className="mt-3 text-center text-[9.5px] prog-serif italic" style={{ color: 'rgba(242,233,216,0.65)' }}>Referee: L. Whitfield · Assistants: M. Doyle, S. Adeyemi · Fourth official: R. Quinn</div>
      </PaperPage>
    )
  }
    ...
    <PaperPage key="tactics" runningHead="How we'll play" folio={String(pages.length + 1).padStart(2, '0')} accent={accent} dark>
      ...
      <div className="mx-auto" style={{ maxWidth: 240 }}>
        <svg viewBox="0 0 300 420" style={{ width: '100%' }}>
          ...
          {([['1', 'Hughes', 150, 392], ['2', 'Ward', 255, 312], ['4', 'Bennett', 197, 330], ['5', 'Stone', 103, 330], ['3', 'Reed', 45, 312], ['6', 'Nair', 150, 250], ['8', 'Osei', 213, 205], ['10', 'Clarke', 87, 205], ['7', 'Whitmore', 248, 110], ['9', 'Carter', 150, 78], ['11', 'Zhang', 52, 110]] as [string, string, number, number][]).map(([num, nm, x, y]) => (
            <g key={num}>
      ...
      <div className="space-y-1.5">
        {['Build patiently from the back and draw Hartwell\'s block forward.', 'Overload the wide areas — full-backs high, wingers inside.', 'Hit the back post from set-pieces — our biggest threat this season.'].map((t, i) => (
  ...
  if (on('spotlight')) P_(
    <PaperPage key="spotlight" runningHead="Player spotlight" folio={String(pages.length + 1).padStart(2, '0')} accent={accent} dark>
      ...
      <div className="prog-serif italic text-[14px] mb-3" style={{ color: P.goldHi }}>Emma Clarke, club centurion</div>
      <div className="my-3"><EditorialPhoto label="Emma Clarke — 100 appearances" h={130} src="/programme/spotlight.jpg" /></div>
      <p className="prog-sans text-[12px] leading-relaxed" style={{ color: 'rgba(242,233,216,0.92)' }}>From the academy to 100 first-team games, Emma has been the heartbeat of this side. “Pulling on this shirt never gets old. The growth of the women’s game here has been unbelievable — I just want to keep winning for these supporters.”</p>
  ...
  if (on('table')) {
    const rows = [
      ['1', 'Castleton Women', '18', '41'], ['2', 'Marsh Rovers W', '18', '38'],
      ['3', 'Oakridge Women', '18', '36'], ['4', 'Penmarric W', '18', '31'],
      ['5', 'Kingsmere W', '18', '29'], ['6', 'Calderbrook W', '18', '27'],
      ['7', 'Hartwell Women', '18', '24'], ['8', 'Thornvale W', '18', '20'],
    ]
    P_(
      <PaperPage key="table" runningHead="League table" folio={String(pages.length + 1).padStart(2, '0')} accent={accent}>
        <Eyebrow color={accent}>Barclays WSL 2</Eyebrow>
        ...
        {rows.map(r => {
          const us = r[1].startsWith('Oakridge'), them = r[1].startsWith('Hartwell')
          return (
        ...
        <div className="text-[9px] mt-2 prog-serif italic" style={{ color: P.muted }}>Top two promoted · as of MD-18</div>
        <SubHead accent={accent}>Division top scorers</SubHead>
        <div className="space-y-1">
          {([['R. Pierce', 'Castleton', '14'], ['E. Clarke', 'Oakridge', '11'], ['A. Friel', 'Marsh Rovers', '10'], ['R. Boyd', 'Hartwell', '9']] as [string, string, string][]).map(([nm, cl, g], i) => {
  ...
  if (on('fixtures')) P_(
    <PaperPage key="fixtures" runningHead="Fixtures & results" folio={String(pages.length + 1).padStart(2, '0')} accent={accent}>
      ...
      <GoldRule />
      {[['W', 'Oakridge 3–1 Kingsmere W'], ['W', 'Penmarric W 0–2 Oakridge'], ['D', 'Oakridge 1–1 Calderbrook W'], ['W', 'Thornvale W 0–4 Oakridge'], ['L', 'Castleton W 2–1 Oakridge']].map((r, i) => (
      ...
      {[['Sat 17 May', 'Marsh Rovers W (A)'], ['Sun 25 May', 'Castleton W (H) · cup'], ['Sat 31 May', 'Penmarric W (H)']].map((r, i) => (
  ...
  if (on('history')) P_(
    <PaperPage key="history" runningHead="On this day" folio={String(pages.length + 1).padStart(2, '0')} accent={accent}>
      ...
      <GoldRule />
      {([['2024', 'Promotion sealed — a 3-0 home win takes Oakridge Women up to WSL 2.'], ['2021', 'Emma Clarke nets her first senior goal in a 2-1 win over Castleton.'], ['2019', 'The club launches its first fully professional women’s squad.'], ['2016', 'A then-record women’s crowd of 4,812 packs Oakridge Stadium.']] as [string, string][]).map(([yr, t]) => (
      ...
      <div className="px-3 py-2.5 text-[11px]" style={{ background: `${accent}0e`, borderLeft: `3px solid ${P.goldHi}`, color: P.ink2 }}>
        WSL 2 runners-up (2023) · County Cup winners (2022, 2025) · Foundation Community Award (2024) · six academy graduates capped at youth international level.
      </div>
    </PaperPage>
  )
  ...
  if (on('community')) P_(
    <PaperPage key="community" runningHead="Around the club" folio={String(pages.length + 1).padStart(2, '0')} accent={accent}>
      ...
      <div className="grid grid-cols-2 gap-2 my-3">
        <EditorialPhoto label="Foundation — girls’ camp" h={90} src="/programme/community-1.jpg" />
        <EditorialPhoto label="Academy U16s — champions" h={90} src="/programme/community-2.jpg" />
      </div>
      <p className="prog-sans text-[12px] leading-relaxed" style={{ color: P.ink2 }}>Our Foundation reached 1,200 girls across local schools this term, and the academy U16s sealed their league title last weekend. The supporters’ club end-of-season awards night is Friday 13 June — tickets on sale now via the club shop.</p>
      <div className="mt-3 px-3 py-2.5 text-[11px]" style={{ background: `${accent}0e`, borderLeft: `3px solid ${P.goldHi}`, color: P.ink2 }}>
        <b className="prog-serif" style={{ color: P.ink }}>Get involved:</b> volunteer matchday roles, junior coaching badges and the “Her Game Too” campaign — speak to any staff member.
      </div>
    </PaperPage>
  )
  ...
  if (on('academy')) P_(
    <PaperPage key="academy" runningHead="Academy focus" folio={String(pages.length + 1).padStart(2, '0')} accent={accent}>
      ...
      <GoldRule />
      <div className="my-3"><EditorialPhoto label="Academy U16s — league champions" h={100} src="/programme/community-2.jpg" /></div>
      <StatStrip accent={accent} items={[['62', 'Academy'], ['9', 'To first team'], ['4', 'In squad today'], ['1,200', 'Girls reached']]} />
      <p className="prog-sans text-[12px] leading-relaxed mt-3" style={{ color: P.ink2 }}>From grassroots centres to the first team, the Oakridge pathway is the beating heart of the club. The U16s are reigning league champions, and three of that group are already training with Sarah Frost’s senior squad this season.</p>
  ...
  if (on('fanzone')) P_(
    <PaperPage key="fanzone" runningHead="Fan zone" folio={String(pages.length + 1).padStart(2, '0')} accent={accent}>
      ...
      <GoldRule />
      <div className="my-3"><EditorialPhoto label="The Rosies faithful at Oakridge Stadium" h={100} src="/programme/fanzone.jpg" /></div>
      ...
      <div className="grid grid-cols-2 gap-3 mt-4">
        <div className="px-3 py-2.5" style={{ border: `1px solid ${P.line}`, background: '#FBF7EE' }}>
          ...
          <div className="prog-serif italic text-[11px]" style={{ color: P.ink2 }}>&ldquo;Oakridge, Oakridge, we'll be with you, in the sun and in the rain…&rdquo;</div>
        </div>
        <div className="px-3 py-2.5" style={{ border: `1px solid ${P.line}`, background: '#FBF7EE' }}>
          <div className="prog-serif text-[12px] font-bold mb-1" style={{ color: accent }}>Fan of the month</div>
          <div className="text-[11px]" style={{ color: P.ink2 }}>Doreen Fields — 40 years a season-ticket holder. A true Rosie.</div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-center gap-5 mt-4 text-[10px] prog-sans" style={{ color: P.muted }}>
        <span>@oakridgewomen</span><span>@OakridgeWFC</span><span>Oakridge Women TV</span>
      </div>
    </PaperPage>
  )
  ...
  if (on('juniors')) P_(
    <PaperPage key="juniors" runningHead="Young supporters" folio={String(pages.length + 1).padStart(2, '0')} accent={accent}>
      <Eyebrow color={accent}>Young Rosies</Eyebrow>
      ...
      {[
        ...
        ['Mascot of the day', 'Today’s mascot is Maisie (age 8) from the Oakridge Juniors U9s. Give her a big cheer as the teams walk out!'],
        ['Quiz', 'How many appearances does Emma Clarke reach today? (Clue: turn to the spotlight page!)'],
      ].map(([t, d], i) => (
  ...
  if (on('sponsors')) P_(
    <PaperPage key="sponsors" runningHead="Our partners" folio={String(pages.length + 1).padStart(2, '0')} accent={accent}>
      ...
      <GoldRule />
      <p className="prog-sans text-[11.5px] mb-3" style={{ color: P.ink2 }}>Our partners make women’s football at Oakridge possible. Please support the businesses that support us.</p>
  ...
  // 14 — BACK COVER
  if (on('back')) P_(
    <div key="back" className="flex-1 flex flex-col relative overflow-hidden prog-sans" style={{ background: `linear-gradient(200deg, #2E0617 0%, ${P.claret} 65%, #9A1452 100%)`, color: P.cream }}>
      <div className="absolute inset-3" style={{ border: `1px solid rgba(203,162,75,0.4)` }} />
      <Grain opacity={0.09} />
      <div className="relative flex flex-col h-full px-8 py-9">
        <div className="flex-1 flex flex-col justify-center">
          <div className="text-[9.5px] font-bold tracking-[0.32em]" style={{ color: P.goldHi }}>NEXT AT OAKRIDGE STADIUM</div>
          <div className="mt-3" style={{ height: 1, width: 60, background: P.goldHi }} />
          <div className="prog-serif mt-4" style={{ fontSize: 30, fontWeight: 900, lineHeight: 1.0 }}>Oakridge Women<br />vs Penmarric</div>
          <div className="text-[12px] mt-3" style={{ color: 'rgba(242,233,216,0.92)' }}>Sat 31 May 2026 · Kick-off 14:00 · WSL 2</div>
          <div className="mt-4 inline-flex w-fit prog-sans text-[11px] font-bold px-3.5 py-2" style={{ background: P.goldHi, color: '#2E0617' }}>Tickets on sale now</div>
        </div>
        <div className="text-center">
          <img src={CREST} alt="" style={{ width: 46, height: 46, margin: '0 auto 8px' }} />
          <div className="prog-serif text-[13px] font-bold tracking-[0.1em]" style={{ color: P.cream }}>UP THE ROSIES</div>
