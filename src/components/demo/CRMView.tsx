...
// ─── Static chart data (never recreated) ────────────────────────────────────

const REVENUE_DATA = [{ month: 'Jan', actual: 34200, target: 38000 }, { month: 'Feb', actual: 38900, target: 40000 }, { month: 'Mar', actual: 42800, target: 42000 }, { month: 'Apr', actual: 41200, target: 44000 }, { month: 'May', actual: 46300, target: 46000 }, { month: 'Jun', actual: 44800, target: 48000 }]
...
const COMPETITOR_ROWS = [{ f: 'AI Deal Scoring', l: '✅ ARIA', h: '❌', p: '❌', s: '⚠️ Einstein $$' }, { f: 'Auto-enrichment', l: '✅ Built-in', h: '⚠️ Add-on', p: '❌', s: '⚠️ Add-on' }, { f: 'Pipeline AI Chat', l: '✅ ARIA Chat', h: '❌', p: '❌', s: '❌' }, { f: 'Ghost Deal Detection', l: '✅', h: '❌', p: '❌', s: '❌' }, { f: 'Price (per seat)', l: '✅ Included', h: '⚠️ £45+', p: '⚠️ £15+', s: '⚠️ £60+' }]
...
function CRMSkeleton() {
  return (
    <div className="space-y-4">
      <div className="h-8 rounded-lg animate-pulse" style={{ background: '#1E2035', width: '60%' }} />
      <div className="grid grid-cols-3 gap-3">{[1,2,3].map(i=><div key={i} className="h-24 rounded-xl animate-pulse" style={{ background: '#1E2035' }} />)}</div>
      <div className="h-48 rounded-xl animate-pulse" style={{ background: '#1E2035' }} />
    </div>
  )
}
...
function CRMViewInner({ company }: { company: string }) {
  ...
  const pipelineValue = useMemo(() => fakeNum(128003, company, 'crmPipe'), [company])
  const openDeals = useMemo(() => fakeNum(34, company, 'crmDeals2'), [company])
  const winRate = useMemo(() => fakeNum(28, company, 'crmWr2'), [company])
  const avgDealSize = useMemo(() => Math.round(pipelineValue / Math.max(1, openDeals)), [pipelineValue, openDeals])
  const revenueThisMonth = useMemo(() => fakeNum(42800, company, 'crmRev'), [company])

  const deals = useMemo(() => Array.from({ length: 12 }, (_, i) => ({
  ...
  const contacts = useMemo(() => Array.from({ length: 9 }, (_, i) => ({
