...
// Cricket portal — Send Message composer (mirrors the Women's composer).
//
// Same 4-step flow as the Women's / Football / Grassroots composers:
...
//
// Oxford-blue themed (cricket accent #3A6CA8). DEMO-SAFE: no real
// messages sent. AI draft uses canned content (standard + urgent
// variants) — no /api/ai/* call on demo clicks.
...
const C = {
  ...
  accDeep:    '#2C5483',
  accLight:   '#7AA7D9',
  accDim:     'rgba(58,108,168,0.18)',
  accBorder:  'rgba(58,108,168,0.5)',
  good:       '#22C55E',
  danger:     '#EF4444',
}
...
const TEAM = [
  ...
  { name: 'Head Coach',          role: 'First XI',             icon: '🧢' },
  { name: 'Director of Cricket', role: 'Cricket operations',   icon: '🏏' },
  { name: 'Club Captain',        role: 'On-field leadership',  icon: '🎯' },
  { name: 'Physio',              role: 'Medical & workload',   icon: '🩺' },
  { name: 'Selectors',           role: 'Selection panel',      icon: '📋' },
  { name: 'Groundsman',          role: 'Pitch & outfield',     icon: '🌱' },
...
const CANNED_STANDARD = `Team — ahead of Saturday's Championship fixture vs Loxwood CC.

First ball 11:00, toss 10:30 at Oakridge Park. Squad is locked — team sheet to the league by Friday. Pitch report: firm, 34% rain risk after 16:00 so we'll plan to bat first if we win the toss. Physio has cleared Hartley (12th man cover). Travel and kit as briefed.

Cheers — Director of Cricket`

const CANNED_URGENT = `[URGENT] Weather call — heavy rain forecast from 14:00 at Oakridge Park.

Groundsman covers on now. Possible early start / reduced overs — selectors and coaches confirm availability for an 10:30 toss decision. Players to arrive 09:30, not 10:00. Reply when received.

Director of Cricket`

type Step = 'who' | 'how' | 'message' | 'preview' | 'sent'

export default function CricketSendMessageModal({ onClose }: { onClose: () => void }) {
        ...
        <div className="p-6 overflow-y-auto flex-1">
          ...
          {step === 'message' && (
            <div className="space-y-4">
              ...
              <p className="text-[10px]" style={{ color: C.text5 }}>Demo: AI draft is canned (no Claude API call). Standard preview uses your text + sign-off; URGENT preview uses a demo weather-call scenario.</p>
            </div>
          )}
