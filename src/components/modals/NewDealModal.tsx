  ...
  return (
    <ModalShell onClose={onClose} onSubmit={() => { if (!dealName || !company || !value || !closeDate) throw new Error('Fill required fields'); onSubmit() }}
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div><Label>Probability (%)</Label><input type="number" value={probability} onChange={e => setProbability(e.target.value)} placeholder="75" min="0" max="100" className="w-full rounded-lg px-3 py-2 text-sm outline-none" style={inputStyle} /></div>
      </div>
