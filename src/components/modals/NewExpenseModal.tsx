...
const CATEGORIES = ['Travel', 'Meals & Entertainment', 'Software & Subscriptions', 'Equipment', 'Office Supplies', 'Marketing', 'Training', 'Other']
const PAYMENT_METHODS = ['Company Card', 'Personal - Reimbursable', 'Cash']
  ...
  return (
    <ModalShell onClose={onClose} onSubmit={() => { if (!amount || !date || !description) throw new Error('Fill required fields'); onSubmit() }}
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div><Label>Payment Method</Label><select value={payment} onChange={e => setPayment(e.target.value)} className="w-full rounded-lg px-3 py-2 text-sm outline-none" style={inputStyle}>{PAYMENT_METHODS.map(p => <option key={p}>{p}</option>)}</select></div>
      </div>
      ...
      {billable && <div><Label>Client Name</Label><input value={clientName} onChange={e => setClientName(e.target.value)} placeholder="Axon Technologies" className="w-full rounded-lg px-3 py-2 text-sm outline-none" style={inputStyle} /></div>}
      <div>
        <Label>Receipt</Label>
        <div className="flex items-center justify-center gap-2 rounded-lg py-4 cursor-pointer transition-colors hover:border-[#374151]" style={{ border: '2px dashed #374151', backgroundColor: 'rgba(255,255,255,0.02)' }}>
          <Upload size={14} style={{ color: '#6B7280' }} /><span className="text-xs" style={{ color: '#6B7280' }}>📎 Attach Receipt</span>
        </div>
      </div>
    </ModalShell>
  )
