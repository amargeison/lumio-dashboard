...
const DEPARTMENTS = ['HR', 'Sales', 'Marketing', 'Operations', 'IT', 'Support', 'Success', 'Engineering', 'Product', 'Finance']
const STATUSES = ['Planning', 'Active', 'On Hold'] as const
const PRIORITIES_P = ['Low', 'Medium', 'High'] as const
...
export default function NewProjectModal({ onClose, onSubmit }: { onClose: () => void; onSubmit: () => void }) {
  ...
  const [priority, setPriority] = useState<typeof PRIORITIES_P[number]>('Medium')
  ...
  return (
    <ModalShell onClose={onClose} onSubmit={() => { if (!name || !owner || !startDate || !description) throw new Error('Fill required fields'); onSubmit() }}
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div><Label required>Project Owner</Label><input value={owner} onChange={e => setOwner(e.target.value)} placeholder="Dan Marsh" className="w-full rounded-lg px-3 py-2 text-sm outline-none" style={inputStyle} /></div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div><Label required>Status</Label><PillSelector options={[...STATUSES]} value={status} onChange={setStatus} colors={{ Planning: '#F59E0B', Active: '#0D9488', 'On Hold': '#9CA3AF' }} /></div>
        <div><Label>Priority</Label><PillSelector options={[...PRIORITIES_P]} value={priority} onChange={setPriority} colors={{ Low: '#22C55E', Medium: '#F59E0B', High: '#EF4444' }} /></div>
      </div>
        ...
        <div className="space-y-2">
          {milestones.map((m, i) => (
            <div key={i} className="grid grid-cols-3 gap-2">
              <input value={m.label} onChange={e => { const n = [...milestones]; n[i].label = e.target.value; setMilestones(n) }} placeholder={`Milestone ${i + 1}`} className="col-span-2 rounded-lg px-3 py-2 text-sm outline-none" style={inputStyle} />
              <input type="date" value={m.date} onChange={e => { const n = [...milestones]; n[i].date = e.target.value; setMilestones(n) }} className="rounded-lg px-3 py-2 text-sm outline-none" style={inputStyle} />
            </div>
          ))}
          {milestones.length < 3 && (
            <button type="button" onClick={() => setMilestones([...milestones, { label: '', date: '' }])} className="flex items-center gap-1 text-xs font-semibold" style={{ color: '#0D9488' }}>
